...
Prompts are personalised with the active investigation subject (`{subject}`, e.g. _MV Ocean Pearl_).
...
Every card routes through the same `onSubmit` path as typed input — the single canonical pipeline (SSOT) is preserved. Clicking a card is a _recommendation_: the officer reviews the wording and presses Enter. **System recommends; officer decides.**
