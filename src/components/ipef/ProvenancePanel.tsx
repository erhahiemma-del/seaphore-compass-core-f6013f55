      ...
      {record.pipelineTrace.map((stage, i) => (
        <div key={stage.contributorId} className="flex items-start gap-3">
          <div className="flex flex-col items-center pt-1.5">
            <div
              className={`h-2 w-2 flex-shrink-0 rounded-full ${STATUS_DOT[stage.status] ?? "bg-slate-300"}`}
            />
          ...
          <div className="flex-1 pb-2 min-w-0">
            ...
            {stage.warnings.map((w, wi) => (
              <p key={wi} className="mt-0.5 text-[10px] text-amber-600">
                ⚠ {w}
              </p>
            ))}
            {stage.errors.map((e, ei) => (
              <p key={ei} className="mt-0.5 text-[10px] text-red-600">
                ✗ {e}
              </p>
            ))}
          </div>
  ...
  return (
    <div className="rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm overflow-hidden">
        ...
        {TABS.map((t) => (
          ...
          >
            ...
            {t.count !== undefined && t.count > 0 && (
              <span className="ml-1 rounded-full bg-slate-200 px-1.5 py-0.5 text-[10px]">
                {t.count}
              </span>
            )}
          </button>
      ...
      <div className="p-4">
        ...
        {tab === "contributors" && (
          <div className="space-y-3">
            {ipef.contributors.map((c) => (
              <ContributorCard key={c.contributorId} c={c} />
            ))}
          </div>
        )}
        {tab === "confidence" && (
          <div className="space-y-3">
            {ipef.confidenceDecompositions.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                No entities processed yet. Send a Copilot query.
              </p>
            ) : (
        ...
        {tab === "lineage" && (
          <div className="space-y-4">
            {ipef.recommendationProvenance.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                No recommendations with lineage in this briefing.
              </p>
            ) : (
                  ...
                  <div className="pl-2 space-y-0.5">
                    {rp.chain.map((node, ni) => (
                      <div key={ni} className="flex items-start gap-2 text-[10px]">
                        ...
                        <span
                          className={`flex-shrink-0 rounded border px-1 ${STATUS_CLS["not-run"]}`}
                        >
                          {node.kind}
                        </span>
      ...
      <div className="border-t border-slate-100 px-5 py-2 flex items-center justify-between">
        <span className="font-mono text-[10px] text-muted-foreground">
          ID: {ipef.correlationId.slice(0, 24)}…
        </span>
