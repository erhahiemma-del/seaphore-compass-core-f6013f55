...
/**
 * Intelligence Centre workspace layout.
 *
 * A content preset, not a shell. It used to wrap `AppShell` and add a
 * second layout on top of it, which made it a competing shell: the five
 * centres using it could not be light, could not opt out of the filter
 * rail, and carried a centre header strip that repeated the screen name
 * the top bar was already showing.
 *
 * It now renders only the working surface — KPI ribbon · workspace tabs ·
 * filters · main · Copilot · evidence tabs · audit bar — and the centre
 * composes it inside `AppShell` like any other content. Layout and
 * spacing stay consistent across the centres; do not fork it per centre.
 *
 * The screen's name is not a prop here. `AppShell` resolves it from the
 * navigation model, so the sidebar and the header cannot disagree.
 */
...
export interface IntelCentreLayoutProps {
  kpiRibbon: ReactNode;
...
export function IntelCentreLayout({
  kpiRibbon,
  tabs,
  activeTab,
  onTabChange,
