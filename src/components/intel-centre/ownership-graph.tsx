...
export function OwnershipGraph({
  ...
  const cx = 260, cy = height / 2, R = Math.min(220, height / 2 - 40);
  ...
  return (
    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px]">
      <div className="rounded-md border border-line/60 bg-surface/40" style={{ height }}>
        <svg viewBox={`0 0 520 ${height}`} className="h-full w-full">
            ...
            return (
              <g key={i}>
                <line
                  ...
                  stroke={edgeColour(e.confidence)} strokeOpacity={0.55} strokeWidth={1.5}
                  strokeDasharray={e.confidence === "inferred" ? "4 3" : undefined}
                />
                <text
                  x={(a.x + b.x) / 2} y={(a.y + b.y) / 2 - 4}
                  textAnchor="middle" fill="#5A6B7B" fontSize={9}
                >
                  {e.label}
                </text>
              </g>
          ...
          {Array.from(positions.entries()).map(([id, p]) => {
            ...
            return (
              <g key={id}>
                <circle cx={p.x} cy={p.y} r={isCenter ? 22 : 16} fill={fill} stroke={stroke} strokeWidth={1.5} />
                <text x={p.x} y={p.y + 4} textAnchor="middle" fill="#E4E8EC" fontSize={9} fontWeight={700}>
                  {kind === "vessel" ? "VSL" : "CO"}
                </text>
                <text x={p.x} y={p.y + (isCenter ? 40 : 32)} textAnchor="middle" fill="#B7C0C8" fontSize={10}>
      ...
      <div className="space-y-2">
        <div className="text-[11px] font-semibold uppercase tracking-[0.06em] text-slate">Edges</div>
        <ul className="space-y-1.5 text-[11px]">
          {shown.map((e, i) => (
            <li key={i} className="flex items-start justify-between gap-2 rounded border border-line/50 bg-surface/50 p-2">
              <div className="min-w-0">
                <div className="truncate font-semibold text-foreground/90">
                  {truncate(nodeLabel(e.fromId), 16)} <span className="text-slate">·</span> {e.label}
                </div>
