...
function ev(
  overrides: Partial<NormalizedEvidence> & { fields?: Record<string, unknown> } = {},
): NormalizedEvidence {
  return {
...
describe("INT-01B · Vessel attribute extractor", () => {
  it("extracts IMO, MMSI, flag from identity evidence", () => {
    const attrs = extractVesselAttributes(
      ev({
        kind: "identity",
        fields: {
          imo: "9438291",
...
describe("INT-01B · Voyage attribute extractor", () => {
  it("extracts port-call fields", () => {
    const attrs = extractVoyageAttributes(
      ev({
        kind: "port-call",
        fields: {
          portName: "Apapa",
          portUnlocode: "NGAPP",
          arrivalTime: "2026-06-01T10:00:00Z",
          speed: 12.5,
        },
      }),
    );
...
describe("INT-01B · Port attribute extractor", () => {
  it("extracts UNLOCODE and coordinates", () => {
    const attrs = extractPortAttributes(
      ev({
        kind: "port-call",
...
describe("INT-01B · Cargo attribute extractor", () => {
  it("extracts HS code, weight, dangerous goods flag", () => {
    const attrs = extractCargoAttributes(
      ev({
        kind: "cargo",
        fields: {
...
describe("INT-01B · Company attribute extractor", () => {
  it("extracts registration number and status", () => {
    const attrs = extractCompanyAttributes(
      ev({
        kind: "identity",
...
describe("INT-01B · Person attribute extractor", () => {
  it("extracts name, nationality, seafarer book", () => {
    const attrs = extractPersonAttributes(
      ev({
        kind: "identity",
        ...
        fields: {
          fullName: "John Doe",
          nationality: "NG",
          seafarerBookNumber: "NG-CDC-12345",
          rank: "Captain",
        },
      }),
    );
...
describe("INT-01B · Sanction attribute extractor", () => {
  it("extracts list name, status, and programme", () => {
    const attrs = extractSanctionAttributes(
      ev({
        kind: "sanctions",
...
describe("INT-01B · Inspection attribute extractor", () => {
  it("extracts inspection type, result, deficiency count", () => {
    const attrs = extractInspectionAttributes(
      ev({
        kind: "inspection",
...
describe("INT-01B · Satellite observation attribute extractor", () => {
  it("extracts scene ID, collection, and bounding box", () => {
    const attrs = extractSatelliteObservationAttributes(
      ev({
        kind: "other",
        fields: {
...
describe("INT-01B · Weather event attribute extractor", () => {
  it("extracts wind speed, wave height, SST", () => {
    const attrs = extractWeatherEventAttributes(
      ev({
        kind: "weather",
        fields: {
          waveHeight: 2.5,
...
describe("INT-01B · IntelligenceObjectRegistry", () => {
  ...
  beforeEach(() => {
    reg = new IntelligenceObjectRegistry();
  });
  ...
  it("stores and retrieves an object by id", () => {
    ...
    expect((obj?.attributes as unknown as Record<string, unknown>).imoNumber).toBe("9438291");
  });
  ...
  it("intelligenceObjects registry is populated after process()", () => {
    const evidence = [
      ev({
        id: "ev_1",
        kind: "identity",
        fields: { imo: "9438291", flag: "NG", vesselType: "Tanker" },
      }),
    ];
    ...
    expect(obj?.objectKind).toBe("vessel");
    expect((obj?.attributes as unknown as Record<string, unknown>).imoNumber).toBe("9438291");
    expect((obj?.attributes as unknown as Record<string, unknown>).flag).toBe("NG");
    expect((obj?.attributes as unknown as Record<string, unknown>).vesselType).toBe("Tanker");
  });
