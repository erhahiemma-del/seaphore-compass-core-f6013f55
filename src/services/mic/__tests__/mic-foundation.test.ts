...
describe("MIC · Confidence model", () => {
  it("micTierFromScore bands correctly", () => {
    expect(micTierFromScore(0.90)).toBe("VERY_HIGH");
    ...
    expect(micTierFromScore(0.40)).toBe("MEDIUM");
    expect(micTierFromScore(0.39)).toBe("LOW");
    expect(micTierFromScore(0.00)).toBe("LOW");
  });

  it("micScoreFromGrade maps every grade", () => {
    expect(micScoreFromGrade("VERIFIED")).toBe(0.95);
    expect(micScoreFromGrade("CORROBORATED")).toBe(0.80);
    expect(micScoreFromGrade("OBSERVED")).toBe(0.65);
    expect(micScoreFromGrade("REPORTED")).toBe(0.45);
    expect(micScoreFromGrade("INFERRED")).toBe(0.30);
    expect(micScoreFromGrade("UNKNOWN")).toBe(0.10);
  });
...
describe("MIC · Entity Registry", () => {
  ...
  it("merges aliases and citations on re-registration", () => {
    mic.entities.register({
      kind: "vessel", canonicalId: VESSEL_ID, label: "MV OCEAN PEARL",
      aliases: ["vessel:mmsi:111"], confidence: "HIGH", grade: "CORROBORATED",
      citations: [{ evidenceId: "ev1", connectorId: "gfw", sourceName: "GFW", grade: "CORROBORATED", observedAt: "2026-01-01T00:00:00Z", excerpt: "A" }],
      sourceUipIds: ["uip_1"],
    });
    mic.entities.register({
      kind: "vessel", canonicalId: VESSEL_ID, label: "MV OCEAN PEARL",
      aliases: ["vessel:mmsi:222"], confidence: "VERY_HIGH", grade: "VERIFIED",
      citations: [{ evidenceId: "ev2", connectorId: "equasis", sourceName: "Equasis", grade: "VERIFIED", observedAt: "2026-02-01T00:00:00Z", excerpt: "B" }],
      sourceUipIds: ["uip_2"],
    });
  ...
  it("resolves aliases to canonical id", () => {
    mic.entities.register({
      kind: "vessel", canonicalId: VESSEL_ID, label: "MV OCEAN PEARL",
      aliases: ["vessel:mmsi:440825000"], confidence: "HIGH", grade: "CORROBORATED",
      citations: [], sourceUipIds: [],
    });
  ...
  it("getByKind returns only matching entities", () => {
    mic.entities.register({
      kind: "vessel", canonicalId: VESSEL_ID, label: "V1",
      aliases: [], confidence: "HIGH", grade: "CORROBORATED",
      citations: [], sourceUipIds: [],
    });
    mic.entities.register({
      kind: "company", canonicalId: COMPANY_ID, label: "C1",
      aliases: [], confidence: "HIGH", grade: "CORROBORATED",
      citations: [], sourceUipIds: [],
    });
...
describe("MIC · Relationship Registry", () => {
  ...
  it("strengthens an edge on re-registration", () => {
    ...
    mic.relationships.register({ ...base, confidence: "HIGH", grade: "CORROBORATED", explanation: "Second source" });
...
describe("MIC · Evidence Registry", () => {
  ...
  it("indexes evidence by entity and UIP", () => {
    mic.evidence.register({
      evidenceId: "ev_a", connectorId: "gfw", sourceName: "GFW",
      grade: "OBSERVED", kind: "position", entityId: VESSEL_ID,
      observedAt: "2026-07-01T00:00:00Z", uipId: "uip_x",
    });
...
describe("MIC · Confidence Registry", () => {
  ...
  it("registers and retrieves a confidence entry", () => {
    mic.confidence.register({
      subjectId: VESSEL_ID, subjectKind: "entity",
      score: 0.82, tier: "HIGH",
  ...
  it("updates confidence on re-registration", () => {
    mic.confidence.register({
      subjectId: VESSEL_ID, subjectKind: "entity",
      score: 0.5, tier: "MEDIUM", components: [],
    });
    mic.confidence.register({
      subjectId: VESSEL_ID, subjectKind: "entity",
      score: 0.9, tier: "VERY_HIGH", components: [],
    });
    const entry = mic.confidence.getForSubject("entity", VESSEL_ID)!;
...
describe("MIC · Timeline Registry", () => {
  ...
  it("registers and retrieves timeline events in chronological order", () => {
    mic.timeline.register({
      kind: "port-visit", label: "Apapa arrival", description: "Port visit",
      entityId: VESSEL_ID, relatedEntityIds: [], occurredAt: "2026-06-15T00:00:00Z",
      citations: [], grade: "OBSERVED", significance: "medium",
    });
    mic.timeline.register({
      kind: "ais-dark", label: "AIS dark event", description: "AIS gap",
      entityId: VESSEL_ID, relatedEntityIds: [], occurredAt: "2026-05-01T00:00:00Z",
      citations: [], grade: "CORROBORATED", significance: "high",
    });
  ...
  it("deduplicates identical events on re-ingestion", () => {
    for (let i = 0; i < 3; i++) {
      mic.timeline.register({
        kind: "port-visit", label: "Same visit", description: "d",
        entityId: VESSEL_ID, relatedEntityIds: [], occurredAt: "2026-06-01T00:00:00Z",
        citations: [], grade: "OBSERVED", significance: "low",
      });
    }
...
describe("MIC · Risk Registry", () => {
  ...
  it("registers a risk profile and retrieves it by entity", () => {
    mic.risk.register({
      entityId: VESSEL_ID, entityLabel: "MV OCEAN PEARL", entityKind: "vessel",
      score: 72, band: "high", confidence: "HIGH",
      indicators: [{
        kind: "sanctions-hit", label: "Sanctions", score: 1.0, weight: 0.30,
        points: 30, rationale: "OFAC hit", citations: [], nodeIds: [VESSEL_ID], confidence: "VERY_HIGH",
      }],
  ...
  it("getByBand returns only matching band", () => {
    mic.risk.register({
      entityId: VESSEL_ID, entityLabel: "V", entityKind: "vessel",
      score: 80, band: "critical", confidence: "HIGH",
      indicators: [], narrative: "", computedAt: "2026-07-01T00:00:00Z",
    });
    mic.risk.register({
      entityId: COMPANY_ID, entityLabel: "C", entityKind: "company",
      score: 10, band: "low", confidence: "MEDIUM",
      indicators: [], narrative: "", computedAt: "2026-07-01T00:00:00Z",
    });
...
describe("MIC · Reasoning Registry", () => {
  ...
  it("registers a reasoning log and retrieves by session", () => {
    mic.reasoning.register({
      sessionId: "sess_001", query: "vessel:imo:1",
      primaryEntityId: VESSEL_ID, statements: [],
      confidence: "HIGH", grade: "CORROBORATED", uipId: "uip_1",
    });
...
describe("MIC · Container — process()", () => {
  ...
  it("processes a UIP and populates all registries", () => {
    const evidence = [
      ...
      makeEvidence({ id: "ev_2", kind: "position", fields: { portName: "Apapa" }, grade: "OBSERVED",
  ...
  it("extracts timeline events from port-visit evidence", () => {
    const uip = makeUip([
      makeEvidence({ id: "ev_p", kind: "position", fields: { portName: "Apapa" }, grade: "OBSERVED" }),
    ]);
    mic.process(uip);
    const events = mic.timeline.getForEntity(VESSEL_ID);
  ...
  it("detects sanctions hit in risk profile", () => {
    const uip = makeUip([
      makeEvidence(),
      makeEvidence({ id: "ev_s", kind: "sanctions", fields: { status: "listed", entityName: "MV OCEAN PEARL" }, grade: "VERIFIED" }),
    ]);
    mic.process(uip);
  ...
  it("stats() returns accurate counts after processing", () => {
    mic.process(makeUip([makeEvidence({ id: "ev_a" }), makeEvidence({ id: "ev_b", kind: "position" })]));
...
describe("MIC · Factory", () => {
  it("createMicContainer returns isolated instances", () => {
    ...
    a.entities.register({
      kind: "vessel", canonicalId: VESSEL_ID, label: "V",
      aliases: [], confidence: "HIGH", grade: "CORROBORATED",
      citations: [], sourceUipIds: [],
    });
