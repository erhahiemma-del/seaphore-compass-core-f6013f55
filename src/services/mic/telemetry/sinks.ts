...
export class ConsoleSink implements MicTelemetrySink {
  emit(t: MicExecutionTelemetry): void {
    const level = t.outcome === "failed" ? "error" : t.outcome === "degraded" ? "warn" : "info";
    console[level](
      "[MIC]",
      JSON.stringify({
        ...
        heapUsedMb: t.heapUsedBytes != null ? +(t.heapUsedBytes / 1_048_576).toFixed(1) : null,
        warnings: t.warnings,
        errors: t.errors,
      }),
    );
  ...
  emit(t: MicExecutionTelemetry): void {
    for (const sink of this.sinks) {
      try {
        ...
        /* one sink must never break another */
      }
    }
  }
}
