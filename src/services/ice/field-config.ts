...
export const FIELD_ALIASES: ReadonlyArray<{ canonical: string; aliases: string[] }> = [
  ...
  { canonical: "flag_state",        aliases: ["flag","flagState","nationality","flagCountry","country_flag"] },
...
export const FRESHNESS_MAX_HOURS: Readonly<Record<string, number>> = {
  eta: 6, lat: 6, lon: 6, speed: 6, heading: 6,
  sanctions_status: 24,
  cargo_weight: 72, cargo_type: 72, hs_code: 72,
  vessel_owner: 168, vessel_manager: 168,
  flag_state: 720, imo_number: 720, mmsi: 720,
...
export const CRITICAL_FIELDS: ReadonlyArray<string> = [
  "imo_number",
  "mmsi",
  "sanctions_status",
];
...
export function fieldQuality(field: string): number {
  const structuredRegistry = new Set(["imo_number","mmsi","hs_code","port_of_call","flag_state"]);
...
export function classifySeverity(
  ...
  if (field === "vessel_owner" || field === "beneficial_owner") return { severity: "HIGH", isCritical: false };
