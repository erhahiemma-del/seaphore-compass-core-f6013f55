...
export interface PolicyEngine {
  can(officer: OfficerContext, workflow: WorkflowId, input: Readonly<Record<string, unknown>>): PolicyDecision;
}

/** Default role matrix — Sprint 10 will replace with policy DSL. */
const ROLE_MATRIX: Readonly<Record<WorkflowId, ReadonlyArray<OfficerContext["role"]>>> = Object.freeze({
...
export const defaultPolicyEngine: PolicyEngine = {
  can(officer, workflow) {
    ...
    return allowed
      ? { allowed: true }
      : { allowed: false, reason: `Role '${officer.role}' is not permitted to trigger '${workflow}'.` };
  },
};
