...
export function brokeredPreviewStorage() {
  ...
  const request = (type: string, key: string, value?: string): Promise<{ ok: boolean; value?: string | null } | null> =>
    new Promise((resolve) => {
      ...
      let done = false;
      /*
       * `let` is required, despite `prefer-const`.
       *
       * The rule sees one assignment and suggests merging it into the
       * declaration. It cannot be merged: `finish` below closes over
       * `timer` to clear it, and the assignment on the timeout line
       * closes over `finish`. Each has to exist before the other is
       * written, so the declaration and the assignment must stay apart.
       */
