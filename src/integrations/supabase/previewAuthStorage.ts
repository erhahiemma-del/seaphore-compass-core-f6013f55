...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      /*
       * `let`, not `const`, and prefer-const is wrong here.
       *
       * `finish` below closes over `timer` to clear it, and is declared
       * before the timer is created — the timeout's own callback calls
       * `finish`, so the two are mutually referential. Declaring it
       * `const` at the assignment site would leave `finish` reading a
       * binding in its temporal dead zone, which happens to work only
       * because nothing calls `finish` earlier. That is an ordering
       * accident, not a guarantee.
       *
       * This file is regenerated by Lovable, which is why the disable is
       * scoped to the one line rather than the rule being switched off.
       */
