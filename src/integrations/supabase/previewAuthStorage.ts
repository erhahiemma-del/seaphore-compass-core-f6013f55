...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      // Declared here rather than above `finish`: `postMessage` queues a
      // task, so no message can be delivered before this line runs, and
      // `finish` therefore cannot read `timer` ahead of its assignment.
