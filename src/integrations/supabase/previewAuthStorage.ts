...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      /*
       * Declared before `finish` so the closure can clear it, assigned
       * after so the callback it schedules can call `finish`. `const` is
       * not available for a binding initialised later, and reordering to
       * satisfy the rule would mean restructuring auth-critical control
       * flow for a lint preference.
       */
      // eslint-disable-next-line prefer-const -- assigned once below; see note.
