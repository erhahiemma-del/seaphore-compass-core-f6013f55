...
function MIOPage() {
  ...
  const [registrySnap, setRegistrySnap]     = useState<Awaited<ReturnType<typeof getMioRegistrySnapshotFn>> | null>(null);
  const [execHistory, setExecHistory]       = useState<Awaited<ReturnType<typeof getMioExecutionHistoryFn>> | null>(null);
  const [pipelineStatus, setPipelineStatus] = useState<Awaited<ReturnType<typeof getMioPipelineStatusFn>> | null>(null);
  const [connectors, setConnectors]         = useState<Awaited<ReturnType<typeof getMioConnectorStatusFn>> | null>(null);
  const [riskDist, setRiskDist]             = useState<Awaited<ReturnType<typeof getMioRiskDistributionFn>> | null>(null);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="sticky top-0 z-10 border-b border-border bg-background/95 px-6 py-3 backdrop-blur">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-foreground">Maritime Intelligence Observatory</h1>
            <p className="text-xs text-muted-foreground">
              Live runtime state · All data from production services · Admin only
              {lastRefresh && <span className="ml-2 text-emerald-600">Last refresh: {lastRefresh}</span>}
            </p>
          </div>
          <button
            onClick={refresh}
      ...
      <div className="p-6">
        ...
        {tab === "pipeline" && (
          <div className="space-y-6">
            <p className="text-sm text-muted-foreground">
              Intelligence pipeline health. MIC stage data is sourced from the telemetry CapturingSink — populated after the first Copilot query.
            </p>
            ...
            {pipelineStatus?.summary && (
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
                ...
                <StatCard label="Success"            value={pipelineStatus.summary.successCount}    sub="outcome=success" />
                <StatCard label="Degraded"           value={pipelineStatus.summary.degradedCount}   sub="with warnings" />
                <StatCard label="Failed"             value={pipelineStatus.summary.failedCount}      sub="outcome=failed" />
            ...
            <div className="rounded-lg border border-border bg-card overflow-hidden">
              <div className="border-b border-border bg-muted/30 px-4 py-2">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Intelligence Pipeline</span>
              </div>
                ...
                {(pipelineStatus?.pipeline ?? []).map((stage) => (
                  <div key={stage.stage} className="flex items-center justify-between px-4 py-3">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      {stage.latencyMs != null && <span className="font-mono">{stage.latencyMs}ms</span>}
            ...
            {(pipelineStatus?.micStages ?? []).length > 0 && (
              <div className="rounded-lg border border-border bg-card overflow-hidden">
                <div className="border-b border-border bg-muted/30 px-4 py-2">
                  <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">MIC Stage Breakdown (last execution)</span>
                </div>
                <div className="divide-y divide-border">
                  {pipelineStatus!.micStages.map((s) => (
                    <div key={s.stage} className="flex items-center justify-between px-4 py-2.5">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="h-2 rounded-full bg-emerald-500/20" style={{ width: `${Math.max(4, Math.min(200, s.durationMs))}px` }} />
                        <span className="font-mono text-xs text-muted-foreground w-14 text-right">{s.durationMs}ms</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {tab === "registries" && (
          <div className="space-y-6">
            <p className="text-sm text-muted-foreground">
              Live state of all 8 MIC registries and the Maritime Knowledge Graph.
              Counts grow as Copilot queries are processed.
            </p>
            {registrySnap ? (
              <>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-lg border border-border bg-card p-4">
                    <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">Knowledge Graph</div>
                    <div className="flex gap-8">
                      <div>
                        <div className="text-3xl font-semibold text-foreground">{registrySnap.graph.nodes}</div>
                        <div className="text-xs text-muted-foreground">Nodes</div>
                      </div>
                      <div>
                        <div className="text-3xl font-semibold text-foreground">{registrySnap.graph.edges}</div>
                  ...
                  <div className="rounded-lg border border-border bg-card p-4">
                    <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">Telemetry Summary</div>
                    {registrySnap.telemetry ? (
                      <dl className="space-y-1.5 text-xs">
                        {[
                          ["Total executions", registrySnap.telemetry.totalExecutions],
                          ["Success / Degraded / Failed", `${registrySnap.telemetry.successCount} / ${registrySnap.telemetry.degradedCount} / ${registrySnap.telemetry.failedCount}`],
                      ...
                      <p className="text-xs text-muted-foreground">No executions yet. Run a Copilot query to populate.</p>
                    )}
                  </div>
                </div>
              </>
        ...
        {tab === "executions" && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Rolling window of the last 100 MIC executions (newest first). Populated by the CapturingSink on every Copilot query.
            </p>
            {execHistory && execHistory.executions.length > 0 ? (
              <div className="rounded-lg border border-border overflow-hidden">
                <table className="min-w-full text-xs">
                  <thead className="bg-muted/50">
                    <tr>
                      {["Time", "Outcome", "Duration", "Entities", "Evidence", "Risk", "Timeline", "Heap MB", "Warnings", "Errors"].map((h) => (
                  ...
                  <tbody className="divide-y divide-border">
                    {execHistory.executions.map((e) => (
                      <tr key={e.executionId} className="hover:bg-muted/20">
                        <td className="px-3 py-2 font-mono text-muted-foreground whitespace-nowrap">{e.timestamp.slice(11, 19)}</td>
                        <td className="px-3 py-2"><StatusBadge status={e.outcome} /></td>
                        ...
                        <td className="px-3 py-2 font-mono">{e.warnings.length > 0 ? <span className="text-amber-600">{e.warnings.length}</span> : "0"}</td>
                        <td className="px-3 py-2 font-mono">{e.errors.length > 0 ? <span className="text-red-600">{e.errors.length}</span> : "0"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="rounded-lg border border-dashed border-border p-10 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Send a Copilot query from the main interface. The MIC will execute automatically and execution data will appear here.
                </p>
              </div>
            )}
          </div>
        ...
        {tab === "risk" && (
          <div className="space-y-6">
            <p className="text-sm text-muted-foreground">
              Risk distribution across all entities in the MIC risk registry. Populated as UIPs are processed.
            </p>
            {riskDist ? (
              <>
                ...
                {riskDist.topRisk.length > 0 && (
                  <div className="rounded-lg border border-border overflow-hidden">
                    <div className="border-b border-border bg-muted/30 px-4 py-2">
                      <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Top Risk Entities</span>
                    </div>
                    <div className="divide-y divide-border">
                        ...
                        <div key={r.entityId} className="px-4 py-3">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-foreground">{r.entityLabel}</span>
                                <span className="rounded border px-1.5 py-0.5 text-[10px] text-muted-foreground">{r.entityKind}</span>
                                <StatusBadge status={r.band} />
                              </div>
                              <p className="mt-0.5 text-xs text-muted-foreground">{r.narrative}</p>
                            </div>
                            <div className="shrink-0 text-right">
                              <div className="text-lg font-bold text-foreground">{r.score.toFixed(0)}</div>
                              <div className="text-[10px] text-muted-foreground">{r.indicators} indicators</div>
                            </div>
                          </div>
                        </div>
                      ))}
        ...
        {tab === "connectors" && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              All registered Evidence Providers from the certified catalog. For live health probes, see Provider Health.
            </p>
            {connectors && (
              <>
                ...
                <div className="rounded-lg border border-border overflow-hidden">
                  <table className="min-w-full text-xs">
                    <thead className="bg-muted/50">
                      <tr>
                        {["Provider", "Sprint", "Authentication", "Credential Env", "Certification"].map((h) => (
                      ...
                      {connectors.providers.map((p) => (
                        <tr key={p.id} className="hover:bg-muted/20">
                          ...
                          <td className="px-3 py-2">
                            <StatusBadge status={p.certification === "CERTIFIED" ? "healthy" : "warning"} />
                          </td>
                        </tr>
                      ))}
                    </tbody>
