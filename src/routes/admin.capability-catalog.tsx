  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Intelligence Capability Catalog</h1>
...
function CatalogView({
  ...
  const domains: Array<"ALL" | CapabilityDomain> = [
    "ALL", "vessel", "cargo", "revenue", "risk", "compliance", "port", "environmental", "operational",
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex flex-wrap gap-1">
          {domains.map((d) => (
            <FilterChip key={d} label={d === "ALL" ? "All domains" : DOMAIN_LABEL[d]} active={domain === d} onClick={() => onDomainChange(d)} />
          ))}
        </div>
        <div className="ml-auto flex gap-1">
          {statuses.map((s) => (
            <FilterChip key={s} label={s === "ALL" ? "All status" : STATUS_META[s].label} active={status === s} onClick={() => onStatusChange(s)} />
          ))}
        </div>
      </div>

      <div className={`grid gap-4 ${selected ? "lg:grid-cols-[minmax(0,1fr)_420px]" : "grid-cols-1"}`}>
        {/* Cards */}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium ${sm.className}`}>
          {sm.dot} {sm.label}
        </span>
      </div>
  ...
  return (
    <aside className="rounded-lg border border-border bg-card overflow-y-auto max-h-[calc(100vh-18rem)] sticky top-6">
      <div className="border-b border-border px-4 py-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="text-xs text-muted-foreground uppercase tracking-wider">{DOMAIN_LABEL[cap.domain]}</div>
      ...
      <div className="divide-y divide-border">
        ...
        <Section title={`KPIs (${cap.kpis.length})`}>
          <ul className="space-y-1.5">
            {cap.kpis.map((k) => (
              <li key={k.label}>
                <div className="text-xs font-medium text-foreground">{k.label}{k.unit ? ` (${k.unit})` : ""}</div>
          ...
          <ul className="space-y-1">
            {cap.copilotFeatures.map((f) => (
              <li key={f} className="text-xs text-muted-foreground flex gap-1.5">
                <span className="shrink-0 text-emerald-500">›</span>{f}
              </li>
            ))}
          </ul>
        </Section>
          ...
          <ul className="space-y-1">
            {cap.oieOutputs.map((o) => (
              <li key={o} className="text-xs text-muted-foreground flex gap-1.5">
                <span className="shrink-0 text-sky-500">›</span>{o}
              </li>
            ))}
          </ul>
        </Section>
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        Dependency matrix — what each capability requires before it can operate.
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="min-w-full text-xs">
          <thead className="bg-muted/50">
            <tr>
              <th className="px-3 py-2 text-left font-medium text-muted-foreground whitespace-nowrap">Capability</th>
          ...
          <tbody className="divide-y divide-border">
            {CAPABILITY_CATALOG.map((cap) => {
              ...
              const requiredBy = DEPENDENCY_MATRIX.filter((e) => e.to === cap.id).map((e) => e.from);
              return (
                <tr key={cap.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-3 py-2">
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${sm.className}`}>
                      {sm.dot} {sm.label}
                    </span>
                  </td>
                  <td className="px-3 py-2">
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="space-y-2 text-xs text-muted-foreground">
          <p>
            <span className="font-medium text-foreground">Foundational (no deps):</span>{" "}
            Vessel Intelligence, Environmental Intelligence — can operate independently once credentials are configured.
          </p>
          <p>
            <span className="font-medium text-foreground">First-order dependents:</span>{" "}
            Cargo Intelligence → Vessel Intelligence. Risk Intelligence → Vessel + Cargo.
            Compliance Intelligence → Vessel Intelligence.
          </p>
          <p>
            <span className="font-medium text-foreground">Second-order dependents:</span>{" "}
            Revenue Intelligence → Cargo Intelligence. Port Intelligence → Vessel Intelligence.
          </p>
          <p>
            <span className="font-medium text-foreground">Orchestration layer:</span>{" "}
            Operational Intelligence depends on all five upstream domains.
          </p>
          <p>
            <span className="font-medium text-foreground">Critical path to full operation:</span>{" "}
            Configure NCS credentials → Cargo Intelligence unlocks → Revenue and Risk Intelligence complete →
            Operational Intelligence achieves full playbook coverage.
          </p>
        </div>
      </div>
    </div>
...
function RoadmapView() {
  const phases = [
    {
      ...
      tone: "emerald",
      items: [
        { label: "Evidence Provider Framework (spec v1.0, cert gate, 9 providers in catalog)", status: "OPERATIONAL" as const },
        { label: "IAL (ConnectorManager, cache, normalization, validation, hashing)", status: "OPERATIONAL" as const },
        { label: "IFE (fusion, conflict detection, source ranking, identity resolution)", status: "OPERATIONAL" as const },
        { label: "Canonical UIP (registry, client store, single-source enforcement)", status: "OPERATIONAL" as const },
        { label: "OIE — 8-module pipeline + 8 playbooks", status: "OPERATIONAL" as const },
        { label: "Environmental Intelligence (Open-Meteo, keyless, live)", status: "OPERATIONAL" as const },
        { label: "OFAC + UNSC sanctions providers (keyless, live)", status: "OPERATIONAL" as const },
        { label: "Copernicus CDSE provider (EP-COPERNICUS-01, OAuth, satellite imagery)", status: "OPERATIONAL" as const },
        { label: "Cargo Workspace — 6 centres + Cargo Copilot + Knowledge Graph", status: "OPERATIONAL" as const },
        { label: "MIBC export — PDF, DOCX, XLSX, PPTX", status: "OPERATIONAL" as const },
        { label: "Dashboard migration — /manifest, /revenue, /cargo → Canonical UIP", status: "OPERATIONAL" as const },
      ],
    },
    {
      ...
      tone: "amber",
      items: [
        { label: "OpenSanctions API key → full sanctions screening across all capabilities", status: "DESIGNING" as const },
        { label: "Equasis account → vessel ISM, class, PSC data for Compliance Intelligence", status: "DESIGNING" as const },
        { label: "IMO GISIS token → authoritative certificate verification", status: "DESIGNING" as const },
        { label: "GFW API token → AIS behaviour, port visits, dark-activity detection", status: "DESIGNING" as const },
        { label: "OpenCorporates token → corporate registry for Ownership chains", status: "DESIGNING" as const },
        { label: "Fix NCS provider ID (id='customs'→'ncs-customs') and projectionContractId", status: "DESIGNING" as const },
    ...
    },
    {
      ...
      tone: "amber",
      items: [
        { label: "NCS/NICIS II API engagement → Cargo Intelligence fully live with VERIFIED-grade data", status: "DESIGNING" as const },
        { label: "NCS credentials activate: manifest intelligence, revenue leakage, cargo Copilot dossier", status: "DESIGNING" as const },
      ],
    },
    {
      ...
      tone: "sky",
      items: [
        { label: "Compliance Intelligence — migrate /compliance from mock data to UIP", status: "PLANNED" as const },
        { label: "Port Intelligence — migrate /ports from mock data to UIP; SAR anchorage monitoring", status: "PLANNED" as const },
        { label: "Vessel Intelligence — migrate /vessel, /ownership from mock data to UIP", status: "PLANNED" as const },
        { label: "Risk Intelligence — national risk KPIs from live evidence (not placeholders)", status: "PLANNED" as const },
        { label: "GFW Satellite Intelligence — AIS-SAR fusion for dark-vessel corroboration", status: "PLANNED" as const },
        { label: "NOAA / Copernicus ECMWF adapter — Environmental Intelligence Source 2+", status: "PLANNED" as const },
      ],
    },
    {
      ...
      tone: "sky",
      items: [
        { label: "All 8 intelligence capabilities at maturity level 5", status: "PLANNED" as const },
        { label: "All credential gates cleared — every provider live", status: "PLANNED" as const },
        { label: "No intel-centre-data.ts mock data in any production route", status: "PLANNED" as const },
        { label: "Test suite: E2E coverage for all capabilities end-to-end", status: "PLANNED" as const },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        Roadmap based on repository evidence and OPS-03 operational certification findings.
        Phase 1 is verified complete. Phases 2–5 are sprint-planning items.
      </p>
      {phases.map(({ phase, tone, items }) => (
        <div key={phase} className="rounded-lg border border-border bg-card overflow-hidden">
          <div className={`px-4 py-3 border-b border-border ${
            ...
            tone === "amber" ? "bg-amber-500/5" : "bg-sky-500/5"
          }`}>
            <h3 className={`text-sm font-semibold ${
              tone === "emerald" ? "text-emerald-700" :
              tone === "amber" ? "text-amber-700" : "text-sky-700"
            }`}>{phase}</h3>
          </div>
