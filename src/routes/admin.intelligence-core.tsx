...
function IntelligenceCoreHealth() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setHealth(h); setStatus(s); setMetrics(m);
    ...
    <div className="min-h-screen bg-background p-6 space-y-6 text-foreground">
      ...
      {health && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
          {[
            ...
            { label: "Feature Flag",  val: <Chip label={health.enabled ? "enabled" : "disabled"} /> },
            { label: "Flag Source",   val: <span className="font-mono text-xs">{health.flagSource}</span> },
            { label: "Graph Live",    val: <Chip label={health.graphLive ? "yes" : "no"} /> },
            { label: "Uptime",        val: `${Math.floor(health.uptimeSeconds / 60)}m ${health.uptimeSeconds % 60}s` },
          ].map(({ label, val }) => (
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Panel title="Registry Status">
          {status ? (
            <dl>
              ...
              <KV k="Executions"     v={`${status.executions.total} total (${status.executions.success} ok / ${status.executions.failed} fail)`} />
            </dl>
        ...
        <Panel title="Performance Metrics">
          {metrics ? (
            <dl>
              <KV k="Avg duration"     v={metrics.performance.avgDurationMs != null ? `${metrics.performance.avgDurationMs}ms` : null} mono />
              <KV k="Min / Max"        v={metrics.performance.minDurationMs != null ? `${metrics.performance.minDurationMs}ms / ${metrics.performance.maxDurationMs}ms` : null} mono />
              <KV k="P50"              v={metrics.performance.p50DurationMs != null ? `${metrics.performance.p50DurationMs}ms` : null} mono />
              <KV k="P95"              v={metrics.performance.p95DurationMs != null ? `${metrics.performance.p95DurationMs}ms` : null} mono />
              <KV k="Heap (last exec)" v={metrics.memory.heapUsedMb != null ? `${metrics.memory.heapUsedMb} MB` : null} mono />
              <KV k="Success rate"     v={metrics.reliability.successRate != null ? `${metrics.reliability.successRate}%` : null} />
              <KV k="Warning rate"     v={metrics.reliability.warningRate != null ? `${metrics.reliability.warningRate}%` : null} />
              <KV k="Failure rate"     v={metrics.reliability.failureRate != null ? `${metrics.reliability.failureRate}%` : null} />
          ...
          <table className="min-w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                <th className="pb-2 text-left font-medium text-muted-foreground">Evidence records</th>
                <th className="pb-2 text-left font-medium text-muted-foreground">Engineering action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {metrics.scalingThresholds.map((t) => (
                <tr key={t.evidenceCount}>
                  <td className="py-1.5 font-mono text-foreground">{t.evidenceCount.toLocaleString()}</td>
