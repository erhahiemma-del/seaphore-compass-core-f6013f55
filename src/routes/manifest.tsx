...
const KPIS: KpiSpec[] = [
  { label: "Today's Manifests",     value: "148",  delta: "+12", trend: "up",   confidence: "verified", series: sparkSeries(3),  emphasis: "risk"    },
  { label: "Pending Validation",    value: "34",   delta: "+6",  trend: "up",   confidence: "observed", series: sparkSeries(7),  emphasis: "warn"    },
  { label: "Duplicates Detected",   value: "5",    delta: "+2",  trend: "up",   confidence: "observed", series: sparkSeries(11)                       },
  { label: "Revenue at Risk",       value: "₦2.14B", delta: "+₦120M", trend: "up", confidence: "inferred", series: sparkSeries(19), emphasis: "risk" },
  { label: "Confidence Score",      value: "82%",  delta: "+1.4%", trend: "up", confidence: "observed", series: sparkSeries(23), emphasis: "ok"     },
  { label: "Open Investigations",   value: "12",   delta: "+3",  trend: "up",   confidence: "verified", series: sparkSeries(29)                       },
  { label: "High Alerts",           value: "5",    delta: "+1",  trend: "up",   confidence: "verified", series: sparkSeries(31), emphasis: "risk"    },
  { label: "Medium / Low Alerts",   value: "2 / 1",              trend: "flat", confidence: "observed", series: sparkSeries(37)                       },
];
      ...
      filters={
        <>
          ...
          <FilterBlock label="Saved views">
            <SavedViewList views={["High risk arrivals", "Duplicates today", "Amended manifests"]} />
          </FilterBlock>
          <FilterBlock label="Time range">
            <CheckList options={["Last 24h", "Last 7d", "Last 30d"]} defaultChecked={["Last 24h"]} />
          </FilterBlock>
          <FilterBlock label="Port">
            <CheckList options={["Apapa", "Tin Can", "Onne", "Port Harcourt", "Calabar"]} defaultChecked={["Apapa", "Tin Can"]} />
          </FilterBlock>
          ...
          <div className="flex flex-wrap items-center gap-1">
            {STATUS_TABS.map((t) => {
              ...
              return (
                ...
                >
                  {t.label}
                  <span className="rounded bg-surface-2/60 px-1.5 py-0.5 text-[10px] text-slate">{count}</span>
                </button>
              );
            })}
          </div>
      ...
      copilot={
        <CentreCopilot
          ...
          observed={[
            ...
            { title: "Repeated agent: Sahara Cargo Nigeria",         detail: "6 of 10 pending manifests use same agent.", confidence: "observed" },
          ]}
          recommendations={[
            { title: "Prioritise Ocean Pearl for manifest review",   detail: "Risk 78 · pending validation · ETA 09:22 UTC.", confidence: "inferred" },
            { title: "Cross-check Delta Star duplicate BOLs",        detail: "3 containers listed under two BOLs (DS-1904 / DS-1907).", confidence: "observed" },
            { title: "Escalate Niger Runner to compliance",          detail: "OFAC SDN watchlist match observed on beneficial owner.", confidence: "verified" },
          ]}
          historical={[
            { title: "Ocean Pearl · Q4 2025",  detail: "Similar risk profile → misclassification confirmed after inspection.", similarity: 84 },
            { title: "Delta Star · Aug 2025",  detail: "Duplicate BOL pattern → ₦96M under-declaration recovered.",           similarity: 71 },
          ]}
          related={[
  ...
  return (
    <div className="rounded-lg border border-line/60 bg-surface/60 p-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <StatusBadge label={v.status.toUpperCase()} tone={v.status === "duplicate" ? "risk" : v.status === "pending" ? "warn" : "ok"} />
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <span className="w-8 text-right text-[11px] font-semibold text-foreground">{v.riskScore}</span>
...
function EvidenceStrip() {
  const cols: Column<(typeof EVIDENCE)[number]>[] = [
    { key: "kind",  label: "Document",  render: (r) => <span className="font-semibold text-foreground">{r.kind}</span> },
    { key: "ref",   label: "Ref #",      render: (r) => <span className="font-mono text-[11.5px]">{r.refNumber}</span> },
    ...
    { key: "conf",  label: "Confidence",align: "right", render: (r) => <ConfidenceChip tier={r.confidence} size={9} /> },
  ];
