...
const KPIS: KpiSpec[] = [
  { label: "Ports Live",           value: String(PORTS.length), delta: "0", trend: "flat", confidence: "verified", series: sparkSeries(3) },
  { label: "Avg Congestion",       value: `${Math.round(PORTS.reduce((a, p) => a + p.congestionIndex, 0) / PORTS.length)}%`, delta: "+3.2%", trend: "up", confidence: "observed", series: sparkSeries(6), emphasis: "warn" },
  { label: "Avg Wait Time",        value: `${Math.round(PORTS.reduce((a, p) => a + p.avgWaitHours, 0) / PORTS.length)}h`, delta: "+1.4h", trend: "up", confidence: "observed", series: sparkSeries(9), emphasis: "warn" },
  { label: "Berths Occupied",      value: "31/44", delta: "+2", trend: "up", confidence: "verified", series: sparkSeries(12) },
  { label: "Arrivals Today",       value: String(PORTS.reduce((a, p) => a + p.todaysEta, 0)), delta: "+4", trend: "up", confidence: "verified", series: sparkSeries(15) },
  { label: "Confidence Score",     value: "86%", delta: "+0.2%", trend: "up", confidence: "observed", series: sparkSeries(18), emphasis: "ok" },
  { label: "Alerts",               value: "5", delta: "+1", trend: "up", confidence: "verified", series: sparkSeries(21), emphasis: "risk" },
];

function PortOpsCentre() {
    ...
    <IntelCentreShell
      ...
      filters={
        <>
                ...
                <li key={p.code}>
                  <button
                    ...
                    className={
                      ...
                      (p.code === selected ? "bg-[color:var(--color-blue)]/15 text-[color:var(--color-blue)]" : "text-foreground/80 hover:bg-surface-2/50")
                    }
                  >
          ...
          <FilterBlock label="Saved views"><SavedViewList views={["Congestion > 70", "Anchorage waits > 24h", "Berth turnover"]} /></FilterBlock>
          <FilterBlock label="Time range"><CheckList options={["Live", "Last 24h", "Last 7d", "Last 30d"]} defaultChecked={["Live"]} /></FilterBlock>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <Section title={`${port.name} · Summary`}>
              ...
              <div className="mt-3">
                <div className="mb-1 text-[10.5px] uppercase tracking-[0.06em] text-slate">Congestion trend · 24h</div>
                <div className="h-10"><Sparkline data={sparkSeries(port.congestionIndex)} trend={port.congestionIndex > 60 ? "up" : "flat"} /></div>
              </div>
            </Section>
            ...
            <Section title="Berth Status (POR-2)">
              <div className="grid grid-cols-4 gap-1.5">
                {Array.from({ length: 12 }).map((_, i) => {
                  const state = i < Math.round(port.congestionIndex / 10) ? "occupied" : i < 10 ? "available" : "maintenance";
                  const colour = state === "occupied" ? "#C0392B" : state === "available" ? "#1E6B3A" : "#5A6B7B";
                  return (
                    <div key={i} className="rounded border border-line/60 bg-surface/50 p-2 text-center">
                      <div className="text-[10px] text-slate">Berth {i + 1}</div>
                      <div className="mx-auto mt-1 h-3 w-3 rounded-sm" style={{ background: colour }} />
          ...
          <Section title="Arrivals · Selected Port">
            <DataTable
              columns={[
                { key: "n", label: "Vessel",   render: (r: typeof arrivals[number]) => <span className="font-semibold text-foreground">{r.name}</span> },
                { key: "t", label: "Type",     render: (r) => r.type },
                { key: "v", label: "Voyage",   render: (r) => <span className="font-mono text-[11.5px]">{r.voyage}</span> },
                { key: "e", label: "ETA",      render: (r) => new Date(r.etaISO).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", timeZone: "UTC" }) + " UTC" },
                { key: "r", label: "Risk",     render: (r) => <StatusBadge label={r.riskLevel.toUpperCase()} tone={r.riskLevel === "high" ? "risk" : r.riskLevel === "medium" ? "warn" : "ok"} /> },
                { key: "c", label: "Confidence", align: "right", render: () => <ConfidenceChip tier="observed" size={9} /> },
              ]}
              rows={arrivals}
              rowKey={(r) => r.id}
              compact
              ...
              {Array.from({ length: 6 }).map((_, i) => {
                ...
                return (
                  <div key={i} className="rounded border border-line/60 bg-surface/50 p-2">
                    <div className="text-[10px] text-slate">+{(i + 1) * 4}h · {String(hour).padStart(2, "0")}:00 UTC</div>
                    <div className="mt-1 text-[16px] font-semibold text-foreground">{Math.round(load)}%</div>
                    <div className="mt-1 h-1 w-full rounded bg-surface-2/50">
                      <div className="h-full rounded" style={{ width: `${load}%`, background: load > 70 ? "#C0392B" : load > 50 ? "#B06A00" : "#1E6B3A" }} />
                    </div>
                  </div>
                );
              })}
      ...
      copilot={
        <CentreCopilot
          ...
          observed={[
            { title: "Apapa congestion trending up",  detail: "Observed +6pts in 6h · anchorage queue at 14 vessels.", confidence: "observed" },
            { title: "Berth 4 idle > 8h at Onne",     detail: "No allocation logged since 02:14 UTC.",                  confidence: "observed" },
            { title: "Weather advisory · Bight",      detail: "Wind advisory may delay pilotage after 18:00 UTC.",      confidence: "inferred" },
          ]}
          recommendations={[
            { title: "Rebalance arrivals to Tin Can", detail: "3 general-cargo arrivals could be re-slotted.", confidence: "inferred" },
            { title: "Notify pilotage of MV Ocean Pearl priority", detail: "High risk, hold on validation.",  confidence: "observed" },
          ]}
          historical={[
            { title: "Apapa peak · Nov 2025", detail: "Same profile → 42h avg wait for 4 days.", similarity: 71 },
          ]}
          related={[
            { ref: "INV-2412-01", title: "Ocean Pearl berth clearance", status: "Open" },
          ]}
        />
      }
    />
