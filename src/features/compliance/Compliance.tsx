...
const MATRIX: ComplianceRow[] = [
  {
    id: "v-ocean-pearl",
    entity: "MV Ocean Pearl",
    subtitle: "IMO 9432187",
    kind: "Vessel",
    certs: "ok",
    inspection: "ok",
    sanctions: "na",
  ...
  {
    id: "v-crimson",
    entity: "MV Crimson Endeavour",
    subtitle: "IMO 9837456",
    kind: "Vessel",
    certs: "fail",
    inspection: "warn",
    sanctions: "fail",
  ...
  {
    id: "v-blue-horizon",
    entity: "MV Blue Horizon",
    subtitle: "IMO 9766453",
    kind: "Vessel",
    certs: "ok",
    inspection: "ok",
    sanctions: "ok",
...
const TIMELINE: TimelineEvent[] = [
  {
    time: "May 27, 2026 09:15",
    title: "ISPS Certificate expired",
    entity: "MV Crimson Endeavour",
    severity: "HIGH",
  },
  {
    time: "May 27, 2026 08:40",
    title: "Arrived Apapa Anchorage",
    entity: "MV Crimson Endeavour",
    severity: "INFO",
  },
  {
    time: "May 26, 2026 14:22",
    title: "Sanctions list updated (OFAC)",
    entity: "Beneficial Owner: John Doe",
    severity: "HIGH",
  },
  {
    time: "May 25, 2026 11:05",
    title: "Inspection scheduled",
    entity: "MV Crimson Endeavour",
    severity: "MEDIUM",
  },
  {
    time: "May 24, 2026 16:30",
    title: "Ownership change detected",
    entity: "MV Crimson Endeavour",
    severity: "MEDIUM",
  },
  {
    time: "May 23, 2026 10:12",
...
function StatusPill({ s }: { s: ComplianceStatus }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center rounded px-1.5 py-0.5 text-[10.5px] font-semibold tracking-wide",
        tone,
      )}
    >
      {s}
...
function SeverityChip({ s }: { s: TimelineEvent["severity"] }) {
  ...
  return (
    <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-semibold tracking-wide", tone)}>
      {s}
    </span>
  );
}
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-line/60 bg-surface-1 p-3">
      ...
      <div className="grid grid-cols-[80px_1fr] gap-y-1.5 text-[11.5px]">
        ...
        <div className="text-[color:var(--color-red)]">Non-Compliant</div>
        <div className="text-slate">Location</div>{" "}
        <div className="text-foreground">Waiting – Apapa Anchorage</div>
        <div className="text-slate">Last Port</div>{" "}
        <div className="text-foreground">Lagos, Nigeria</div>
        <div className="text-slate">ETA</div>{" "}
        <div className="text-foreground">May 27, 2026 10:40</div>
      </div>
      ...
      <div className="mt-3">
        ...
        <ul className="mt-1.5 space-y-1.5">
          {insights.map((i) => (
            <li
              key={i.text}
              className="flex items-start justify-between gap-2 text-[11.5px] text-foreground/90"
            >
  ...
  return (
    <div className="rounded-lg border border-line/60 bg-surface-1 p-3">
      <div className="flex items-center justify-between">
        ...
        <button className="text-[11px] text-[color:var(--color-blue)] hover:underline">
          View all
        </button>
      </div>
      <div className="mt-2 grid grid-cols-2 gap-2">
        {EVIDENCE_SNAPSHOT.map((e) => {
          ...
          return (
            <div
              key={e.label}
              className="flex items-center gap-2 rounded-md border border-line/50 bg-surface-2/40 p-2"
            >
...
export function ComplianceCentre() {
  ...
  const mapEntities = useMemo(() => buildComplianceMapEntities(deferredMask), [deferredMask]);
                ...
                <ul className="mt-2 space-y-1.5">
                  {MAP_LAYERS.map((l) => {
                    ...
                    return (
                      <li
                        key={l.key}
                        className="flex items-center justify-between text-[11.5px] text-foreground/90"
                      >
                        <span className="flex items-center gap-1.5">
                          <span
                ...
                <div className="mt-3 border-t border-line/50 pt-2">
                  ...
                  <ul className="mt-1.5 space-y-1 text-[11.5px] text-foreground/90">
                    <li className="flex items-center gap-1.5">
                      <span className="h-2.5 w-2.5 rounded-sm bg-[color:var(--color-red)]" />
                      High
                    </li>
                    <li className="flex items-center gap-1.5">
                      <span className="h-2.5 w-2.5 rounded-sm bg-[color:var(--color-amber)]" />
                      Medium
                    </li>
                    <li className="flex items-center gap-1.5">
                      <span className="h-2.5 w-2.5 rounded-sm bg-[color:var(--color-green)]" />
                      Low
                    </li>
                  </ul>
                </div>
                  ...
                  <table className="w-full border-t border-line/50 text-[11.5px]">
                    ...
                    <tbody className="text-foreground/90">
                      {MATRIX.map((r) => (
                        <tr
                          key={r.id}
                          className="border-t border-line/40 hover:bg-surface-2/40 [&>td]:px-3 [&>td]:py-2"
                        >
                          <td>
                          ...
                          <td className="text-center">
                            <div className="inline-flex">
                              <StatusDot v={r.inspection} />
                            </div>
                          </td>
                          <td className="text-center">
                            <div className="inline-flex">
                              <StatusDot v={r.sanctions} />
                            </div>
                          </td>
                          <td className="tabular-nums">{r.regulations}</td>
                          <td>
                            <ScoreBar pct={r.score} status={r.status} />
                          </td>
                          <td>
            ...
            <div className="grid gap-3 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
              <section className="rounded-lg border border-line/60 bg-surface-1">
                <header className="flex items-center justify-between px-3 py-2">
                  ...
                  <button className="text-[11px] text-slate hover:text-foreground">
                    Select Regulation ▾
                  </button>
                </header>
                <div className="grid grid-cols-3 gap-2 border-t border-line/50 p-3 md:grid-cols-4 xl:grid-cols-7">
                  {REGULATIONS.map((r) => {
                    ...
                    return (
                      <div
                        key={r.code}
                        className="rounded-md border border-line/50 bg-surface-2/40 p-2 text-center"
                      >
                        <div
                          className={cn(
                        ...
                        <div className="text-[10.5px] text-slate tabular-nums">
                          {r.compliant} / {r.total}
                        </div>
                        <div
                          className={cn(
