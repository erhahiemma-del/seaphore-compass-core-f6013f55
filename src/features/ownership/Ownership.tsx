...
export function OwnershipCentre() {
  ...
  const tabEntities = useMemo(() => {
    ...
    const confRank: Record<string, number> = { verified: 95, observed: 80, inferred: 65, unconfirmed: 40 };

    if (tab === "companies" || tab === "all") {
      const rows = COMPANIES.filter((c) => passSearch(c.name) && (confRank[c.verified] ?? 50) >= confMin);
      if (tab === "companies") return rows.map((c) => ({ id: c.id, name: c.name, kind: "Company", meta: c.role, country: c.country, tier: c.verified }));
    ...
    if (tab === "persons") {
      return PERSONS.filter((p) => passSearch(p.name)).map((p) => ({ id: p.id, name: p.name, kind: "Person", meta: p.role, country: p.country, tier: p.verified }));
    }
    if (tab === "vessels") {
      return VESSELS.filter((v) => passSearch(v.name)).map((v) => ({ id: v.id, name: v.name, kind: "Vessel", meta: v.type, country: v.flag, tier: "verified" as const }));
    }
    if (tab === "directors") {
      return PERSONS.filter((p) => p.role === "Director" && passSearch(p.name)).map((p) => ({ id: p.id, name: p.name, kind: "Director", meta: `→ ${companyById(p.companyId)?.name ?? ""}`, country: p.country, tier: p.verified }));
    }
    return PERSONS.filter((p) => p.role === "Shareholder" && passSearch(p.name)).map((p) => ({ id: p.id, name: p.name, kind: "Shareholder", meta: `${p.stakePct ?? "?"}% · ${companyById(p.companyId)?.name ?? ""}`, country: p.country, tier: p.verified }));
  }, [tab, deferredSearch, confMin]);

  const filteredInvestigations = useMemo(() => {
    if (risk === "all") return RELATED_INVESTIGATIONS;
    return RELATED_INVESTIGATIONS.filter((i) =>
      risk === "high" ? i.risk === "High" : risk === "medium" ? i.risk === "Medium" : i.risk === "Low",
    );
  }, [risk]);
  ...
  const selectEntity = (id: string) => {
    startTransition(() => setSelectedId(id.startsWith("port-") ? activeSelectedId : id.startsWith("p-") ? activeSelectedId : id));
  };
  ...
  return (
    <AppShell title="Ownership Intelligence Centre" subtitle="Uncover true ownership. Map relationships. Detect risk." mode="dark">
      <div className="flex min-h-[calc(100vh-8rem)] flex-col bg-background text-foreground">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line/60 bg-surface/40 px-5 py-3">
          <div className="flex min-w-[280px] max-w-[560px] flex-1 items-center gap-1.5 rounded-md border border-line/60 bg-surface-2/40 px-3 py-1.5">
            ...
            />
            <span className="rounded border border-line/60 px-1.5 py-0.5 text-[9.5px] text-slate">/</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className="relative rounded-md border border-line/60 bg-surface-2/40 p-1.5 text-slate hover:text-foreground" aria-label="Notifications">
              ...
              <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[color:var(--color-red)] text-[9px] font-bold text-white">12</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-b border-line/60 bg-surface/40 px-5 py-3 sm:grid-cols-4 xl:grid-cols-8">
          {OWNERSHIP_KPIS.map((k) => {
            ...
            return (
              <div key={k.label} className="rounded-lg border border-line/60 bg-surface-1 px-3 py-2.5">
                <div className="flex items-start gap-2.5">
                  <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-md", TONE_BG[k.tone])}>
                    <Icon className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="text-[10px] uppercase leading-tight tracking-[0.05em] text-slate">{k.label}</div>
                    <div className="mt-0.5 text-[20px] font-semibold leading-none text-foreground">{k.value}</div>
                    <div className={cn("mt-1 text-[10px]", k.trend === "flat" ? "text-slate" : "text-[color:var(--color-green)]")}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line/60 bg-surface/40 px-5 py-2">
          ...
          <div className="flex items-center gap-2 text-[11.5px]">
            <Select label="Risk" value={risk} onChange={(v) => setRisk(v as RiskFilter)} options={[
            ...
            <label className="flex items-center gap-1.5 rounded-md border border-line/60 bg-surface-2/40 px-2 py-1 text-slate">
              ...
              <input type="range" min={30} max={95} step={5} value={confMin} onChange={(e) => setConfMin(Number(e.target.value))} className="w-16 accent-[color:var(--color-blue)]" />
            </label>
            <Select label="Time Range" value={time} onChange={(v) => setTime(v as TimeFilter)} options={[
              { value: "all", label: "All" }, { value: "30d", label: "30 days" }, { value: "90d", label: "90 days" }, { value: "1y", label: "1 year" }, { value: "5y", label: "5 years" },
            ]} />
            <button
              onClick={() => { setRisk("all"); setConfMin(60); setTime("all"); setSearch(""); }}
          ...
          <div className="min-w-0 space-y-3">
            <div className="rounded-lg border border-line/60 bg-surface/60">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line/60 px-3 py-2">
                <div className="flex items-center gap-2">
                  <h3 className="text-[12px] font-semibold uppercase tracking-[0.06em] text-slate">Ownership Network Graph</h3>
                  <span className="inline-flex items-center gap-1 text-[10.5px] text-[color:var(--color-green)]">
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[color:var(--color-green)]" /> LIVE
                  </span>
                </div>
                <div className="flex items-center gap-2 text-[11.5px]">
                  <label className="flex items-center gap-1.5 rounded-md border border-line/60 bg-surface-2/40 px-2 py-1 text-slate">
                    <span>Layout:</span>
                    <select value={layout} onChange={(e) => setLayout(e.target.value as GraphLayout)} className="bg-transparent text-foreground outline-none">
                      {LAYOUTS.map((l) => <option key={l.key} value={l.key} className="bg-background">{l.label}</option>)}
                    </select>
                  </label>
                </div>
              </div>
              <div className="grid gap-2 p-2 md:grid-cols-[190px_minmax(0,1fr)]">
                ...
                <div className="space-y-3 rounded-md border border-line/60 bg-surface/40 p-2 text-[11.5px]">
                  <div>
                    <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">Show / Hide</div>
                    {([
                      ["company", "Companies"], ["person", "Persons"], ["vessel", "Vessels"], ["port", "Ports"],
                    ] as [GraphNodeKind, string][]).map(([k, label]) => (
                      <label key={k} className="flex items-center gap-2 py-0.5 text-foreground/85">
                        <input type="checkbox" checked={visibleKinds[k]} onChange={(e) => setVisibleKinds((v) => ({ ...v, [k]: e.target.checked }))} className="h-3 w-3 accent-[color:var(--color-blue)]" />
                        {label}
                      </label>
                    ))}
                    <label className="flex items-center gap-2 py-0.5 text-foreground/85">
                      <input type="checkbox" checked={visibleRelations["subsidiary-of"]} onChange={(e) => setVisibleRelations((v) => ({ ...v, "subsidiary-of": e.target.checked }))} className="h-3 w-3 accent-[color:var(--color-blue)]" />
                      Directors
                    </label>
                    <label className="flex items-center gap-2 py-0.5 text-foreground/85">
                      <input type="checkbox" checked={visibleRelations["beneficial-owner"]} onChange={(e) => setVisibleRelations((v) => ({ ...v, "beneficial-owner": e.target.checked }))} className="h-3 w-3 accent-[color:var(--color-blue)]" />
                      Shareholders
                    </label>
                    <label className="flex items-center gap-2 py-0.5 text-foreground/85">
                      <input type="checkbox" checked={visibleRelations["agent-of"]} onChange={(e) => setVisibleRelations((v) => ({ ...v, "agent-of": e.target.checked }))} className="h-3 w-3 accent-[color:var(--color-blue)]" />
                      Cargo Relationships
                    </label>
                  </div>
                  <div>
                    <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">Relationship Types</div>
                    {([
                    ...
                    ] as [OwnershipEdge["label"], string, string][]).map(([k, label, colour]) => (
                      <label key={k} className="flex items-center gap-2 py-0.5 text-foreground/85">
                        <input type="checkbox" checked={visibleRelations[k]} onChange={(e) => setVisibleRelations((v) => ({ ...v, [k]: e.target.checked }))} className="h-3 w-3 accent-[color:var(--color-blue)]" />
              ...
              <div className="flex items-center gap-3 border-t border-line/60 px-3 py-2 text-[11px]">
                <input
                  type="range" min={2013} max={2026} step={1}
                  value={asOfYear}
                ...
                <div className="flex items-center gap-1">
                  {(["1Y", "2Y", "5Y", "All"] as const).map((r) => (
                    <button key={r}
                      onClick={() => setAsOfYear(r === "All" ? 2026 : 2026 - (r === "1Y" ? 1 : r === "2Y" ? 2 : 5))}
                      className={cn("rounded px-2 py-0.5 text-[10.5px]",
                        (r === "All" ? asOfYear === 2026 : asOfYear === 2026 - (r === "1Y" ? 1 : r === "2Y" ? 2 : 5))
                          ? "bg-[color:var(--color-blue)]/20 text-[color:var(--color-blue)]"
                          : "text-slate hover:text-foreground")}
                    >{r}</button>
                  ))}
                </div>
                <span className="w-16 text-right font-mono text-[10.5px] text-slate">as of {asOfYear}</span>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-line/60 bg-surface/60">
              <div className="flex items-center justify-between border-b border-line/60 px-3 py-2">
                <h3 className="text-[12px] font-semibold uppercase tracking-[0.06em] text-slate">Ownership Timeline</h3>
                <button className="text-[11.5px] text-[color:var(--color-blue)] hover:underline">View full timeline →</button>
              </div>
              <div className="scrollbar-thin flex gap-3 overflow-x-auto px-3 py-3">
                {timelineEvents.map((e) => (
                  <div key={e.id} className="min-w-[170px] shrink-0 rounded-md border border-line/60 bg-surface/50 p-2.5">
                    ...
                    <div className="mt-1"><ConfidenceChip tier={e.confidence} size={9} /></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid gap-3 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
              <div className="rounded-lg border border-line/60 bg-surface/60">
                <div className="flex items-center justify-between border-b border-line/60 px-3 py-2">
                  <h3 className="text-[12px] font-semibold uppercase tracking-[0.06em] text-slate">Related Investigations</h3>
                  <button className="text-[11.5px] text-[color:var(--color-blue)] hover:underline">View all investigations →</button>
                </div>
              ...
              <div className="rounded-lg border border-line/60 bg-surface/60">
                <div className="flex items-center justify-between border-b border-line/60 px-3 py-2">
                  <h3 className="text-[12px] font-semibold uppercase tracking-[0.06em] text-slate">Supporting Evidence</h3>
                  <button className="text-[11.5px] text-[color:var(--color-blue)] hover:underline">View all →</button>
                </div>
                <div className="grid grid-cols-2 gap-2 p-3">
                  {SUPPORTING_EVIDENCE.map((ev) => {
                    const Icon = EVIDENCE_ICONS[ev.key] ?? FileText;
                    return (
                      <button key={ev.key} className="flex items-start gap-2 rounded-md border border-line/60 bg-surface/50 p-2 text-left hover:bg-surface/80">
                        <Icon className="mt-0.5 h-4 w-4 text-[color:var(--color-blue)]" />
                        <div className="min-w-0">
                          <div className="truncate text-[11.5px] font-semibold text-foreground">{ev.label}</div>
                  ...
                  <li key={e.id}>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="truncate text-[10.5px] text-slate">{e.meta} · {e.country}</div>
                      </div>
                      <div className="flex shrink-0 items-center gap-1">
                        <span className="rounded bg-surface-2/60 px-1.5 py-0.5 text-[9.5px] uppercase text-slate">{e.kind}</span>
          ...
          <div className="space-y-3">
            <div className="rounded-lg border border-line/60 bg-surface/60 p-3">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.08em]" style={{ color: "#7C3AED", backgroundColor: "#7C3AED22" }}>BETA</span>
                <button className="ml-auto rounded p-0.5 text-slate hover:text-foreground" aria-label="More">
                  <MoreHorizontal className="h-3.5 w-3.5" />
                </button>
              </div>
              <div className="mt-2 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">Ask Copilot</div>
              <form
                className="mt-1 flex items-center gap-1.5 rounded border border-line/60 bg-background/40 px-2 py-1.5"
                onSubmit={(e) => { e.preventDefault(); setAskOpen(true); }}
              >
                <input
                  value={askSeed}
                ...
                />
                <button type="submit" className="rounded bg-[color:var(--color-blue)] p-1 text-white hover:opacity-90" aria-label="Send">
            ...
            <div className="rounded-lg border border-line/60 bg-surface/60 p-3">
              <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">Key Insights</div>
                ...
                {KEY_INSIGHTS.map((k, i) => (
                  <li key={i} className="flex items-start gap-2 text-[11.5px]">
                    <span className={cn("mt-1 h-1.5 w-1.5 rounded-full",
                      k.severity === "HIGH" ? "bg-[color:var(--color-red)]" : k.severity === "MEDIUM" ? "bg-[color:var(--color-amber)]" : "bg-[color:var(--color-green)]")} />
            ...
            <div className="rounded-lg border border-line/60 bg-surface/60 p-3">
              <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">Recommended Actions</div>
              <ul className="space-y-1.5">
                {RECOMMENDED_ACTIONS.map((r, i) => (
                  <li key={i} className="flex items-start gap-2 rounded p-1 text-[11.5px] hover:bg-surface-2/40">
            ...
            <div className="rounded-lg border border-line/60 bg-surface/60 p-3">
              <div className="mb-1.5 flex items-center justify-between text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">
                ...
                <button className="text-[color:var(--color-blue)] normal-case hover:underline">View all</button>
              </div>
              <ul className="space-y-1.5 text-[11.5px]">
                {SIMILAR_NETWORKS.map((n) => (
                  <li key={n.name} className="flex items-center justify-between gap-2 rounded p-1 hover:bg-surface-2/40">
            ...
            <div className="rounded-lg border border-line/60 bg-surface/40 p-2.5">
              <div className="mb-1 text-[10px] font-semibold uppercase tracking-[0.06em] text-slate">Cross-links</div>
              <div className="flex flex-wrap gap-1.5 text-[11px]">
                {[
                  ...
                  <a key={label} href={href} className="inline-flex items-center gap-1 rounded border border-line/60 bg-surface-2/50 px-2 py-1 text-foreground/85 hover:bg-surface-2/80">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line/60 bg-surface-2/50 px-5 py-2.5 text-[11.5px]">
          ...
          <a href="/admin" className="inline-flex items-center gap-1 rounded text-[11.5px] font-medium text-[color:var(--color-blue)] hover:underline">
      ...
      <AskCopilotDialog
        instance="ownership"
        open={askOpen}
        onOpenChange={(o) => { setAskOpen(o); if (!o) setAskSeed(""); }}
        seedQuery={askSeed}
      />
    </AppShell>
  );
...
function EntityProfile({
  company, personCount, directorCount, shareholderCount, subsidiaryCount, linkedVessels, linkedPorts, riskScore, confidencePct,
}: {
  company: { id: string; name: string; role: string; country: string; cacNumber?: string; verified: "verified" | "observed" | "inferred" | "unconfirmed" };
  personCount: number; directorCount: number; shareholderCount: number; subsidiaryCount: number;
  linkedVessels: number; linkedPorts: number; riskScore: number; confidencePct: number;
}) {
  ...
  return (
    <div className="rounded-lg border border-line/60 bg-surface/60">
      <div className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <h3 className="text-[12px] font-semibold uppercase tracking-[0.06em] text-slate">Entity Profile</h3>
      ...
      <div className="space-y-3 p-3">
        ...
        <dl className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px]">
          ...
          <Field k="Risk Score" v={
            <span className="inline-flex items-center gap-1">
              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[color:var(--color-red)]/20 text-[9.5px] font-bold text-[color:var(--color-red)]">{riskScore}</span>
              <span className="text-[color:var(--color-red)] font-semibold">{risk} Risk</span>
            </span>
          } />
          <Field k="Confidence" v={
            <span className="inline-flex items-center gap-1">
              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[color:var(--color-green)]/20 text-[9px] font-bold text-[color:var(--color-green)]">{confidencePct}</span>
        ...
        <div className="border-t border-line/60 pt-2">
          <ul className="space-y-1 text-[11px]">
            <Row icon={<UserCheck className="h-3.5 w-3.5 text-[color:var(--color-blue)]" />} label="Beneficial Owners" value={personCount || 2} />
            <Row icon={<UserCheck className="h-3.5 w-3.5 text-[color:var(--color-blue)]" />} label="Directors" value={directorCount || 5} />
            <Row icon={<UserCheck className="h-3.5 w-3.5 text-[color:var(--color-blue)]" />} label="Shareholders" value={shareholderCount || 7} />
            <Row icon={<Landmark className="h-3.5 w-3.5 text-[color:var(--color-blue)]" />} label="Subsidiaries" value={subsidiaryCount} />
            <Row icon={<Ship className="h-3.5 w-3.5 text-[color:var(--color-blue)]" />} label="Linked Vessels" value={linkedVessels} />
            <Row icon={<Anchor className="h-3.5 w-3.5 text-[color:var(--color-blue)]" />} label="Linked Ports" value={linkedPorts} />
          </ul>
        </div>
        <a href="/memory" className="inline-flex w-full items-center justify-center gap-1 rounded-md border border-primary/40 bg-primary/10 px-2 py-1.5 text-[11.5px] font-semibold text-primary hover:bg-primary/15">
          View Full Profile →
        </a>
      </div>
    </div>
...
function Row({ icon, label, value }: { icon: React.ReactNode; label: string; value: React.ReactNode }) {
  return (
    <li className="flex items-center justify-between gap-2">
      <span className="inline-flex items-center gap-1.5 text-foreground/80">{icon}{label}</span>
...
function RiskTag({ risk }: { risk: "High" | "Medium" | "Low" }) {
  const c = risk === "High" ? "text-[color:var(--color-red)]" : risk === "Medium" ? "text-[color:var(--color-amber)]" : "text-[color:var(--color-green)]";
...
function StatusTag({ status }: { status: "Open" | "In Progress" | "Escalated" | "Closed" }) {
  ...
  return <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.06em]", tone)}>{status}</span>;
}
function SeverityTag({ s }: { s: "HIGH" | "MEDIUM" | "LOW" }) {
  const tone = s === "HIGH"
    ...
    : s === "MEDIUM"
      ...
      : "bg-[color:var(--color-green)]/15 text-[color:var(--color-green)]";
  return <span className={cn("rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.06em]", tone)}>{s}</span>;
}
