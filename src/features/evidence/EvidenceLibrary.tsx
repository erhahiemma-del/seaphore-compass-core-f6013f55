...
type TabKey =
  | "overview" | "evidence" | "documents" | "media"
  | "ais" | "manifests" | "bills" | "relationships"
  | "investigations" | "timeline" | "audit";
...
function TopSearch({ query, onQuery, onUpload }: { query: string; onQuery: (s: string) => void; onUpload: () => void }) {
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-lg border border-line/60 bg-surface-1/70 px-3 py-2">
      <div className="relative flex-1 min-w-[280px]">
        ...
        />
        <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rounded border border-line/60 bg-surface-2/60 px-1.5 py-[1px] text-[10px] text-slate">/</span>
      </div>
...
function KpiRibbon({ items }: { items: EvidenceItem[] }) {
  ...
  const tiles: { icon: React.ReactNode; tone: string; label: string; value: string; delta: string }[] = [
    { icon: <FileText className="h-4 w-4" />, tone: "text-emerald-400", label: "Evidence Items", value: fmt(24583), delta: "+12% vs last 30 days" },
    { icon: <FileText className="h-4 w-4" />, tone: "text-[color:var(--color-blue)]", label: "Documents", value: fmt(scale(c("Documents"), 900)), delta: "+8% vs last 30 days" },
    { icon: <ImageIcon className="h-4 w-4" />, tone: "text-purple-400", label: "Images / Media", value: fmt(4671), delta: "+15% vs last 30 days" },
    { icon: <Radio className="h-4 w-4" />, tone: "text-sky-400", label: "AIS Records", value: fmt(6321), delta: "+9% vs last 30 days" },
    { icon: <ClipboardList className="h-4 w-4" />, tone: "text-rose-400", label: "Manifests", value: fmt(3245), delta: "+11% vs last 30 days" },
    { icon: <FileBadge className="h-4 w-4" />, tone: "text-indigo-400", label: "Bills of Lading", value: fmt(2847), delta: "+7% vs last 30 days" },
    { icon: <ShieldCheck className="h-4 w-4" />, tone: "text-emerald-400", label: "Verified Evidence", value: fmt(18642), delta: `${Math.round((verified / total) * 100)}% of total` },
    { icon: <FolderOpen className="h-4 w-4" />, tone: "text-amber-400", label: "Evidence in Active Cases", value: fmt(1243), delta: "+6% vs last 30 days" },
  ];
  return (
      ...
      {tiles.map((t) => (
        <div key={t.label} className="rounded-lg border border-line/60 bg-surface-1/70 p-3">
          <div className="flex items-center gap-2 text-[10.5px] uppercase tracking-[0.06em] text-slate">
            <span className={cn("flex h-6 w-6 items-center justify-center rounded-md bg-surface-2/60", t.tone)}>{t.icon}</span>
            {t.label}
          </div>
...
function ViewSwitcher({ view, setView }: { view: "cards" | "grid" | "list"; setView: (v: "cards" | "grid" | "list") => void }) {
  const btn = (k: "cards" | "grid" | "list", icon: React.ReactNode) => (
    <button
      ...
      className={cn(
        ...
        view === k ? "bg-[color:var(--color-blue)]/20 text-[color:var(--color-blue)]" : "text-slate hover:bg-surface-2/60",
      )}
    >
      {icon}
    </button>
...
function EvidenceExplorer({
  view, evidence, selectedId, onSelect,
}: {
  view: "cards" | "grid" | "list"; evidence: EvidenceItem[]; selectedId: string;
  ...
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        ...
        <button className="text-slate hover:text-foreground"><MoreVertical className="h-3.5 w-3.5" /></button>
      </header>
      <div className={cn(
        "gap-3 p-3",
        ...
        {items.map((e) => (
          <ExplorerCard key={e.id} item={e} selected={e.id === selectedId} onClick={() => onSelect(e.id)} compact={view === "list"} />
        ))}
      </div>
      <footer className="flex items-center justify-between border-t border-line/60 px-3 py-2 text-[11px] text-slate">
        <span>Showing 1 – {items.length} of {fmt(evidence.length)}</span>
        <div className="flex items-center gap-1">
          <button className="rounded p-1 hover:bg-surface-2/60"><ChevronLeft className="h-3 w-3" /></button>
          {["1", "2", "3", "…", "2049"].map((n, i) => (
            <button key={i} className={cn("min-w-[22px] rounded px-1.5 py-0.5 text-[11px]", n === "1" ? "bg-[color:var(--color-blue)]/20 text-[color:var(--color-blue)]" : "hover:bg-surface-2/60")}>
              {n}
            </button>
          ))}
          <button className="rounded p-1 hover:bg-surface-2/60"><ChevronRight className="h-3 w-3" /></button>
        </div>
      </footer>
    </section>
  );
...
function ExplorerCard({ item, selected, onClick, compact }: { item: EvidenceItem; selected: boolean; onClick: () => void; compact?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "group flex flex-col overflow-hidden rounded-md border text-left transition-all",
        selected ? "border-[color:var(--color-blue)] bg-[color:var(--color-blue)]/10 ring-1 ring-[color:var(--color-blue)]/30" : "border-line/60 bg-surface-2/40 hover:bg-surface-2/60",
    ...
    >
      <div className={cn(
        ...
        <span className="flex items-center gap-1"><span className="text-[color:var(--color-blue)]">{iconForKind(item.kind)}</span> {item.kind}</span>
      </div>
      <div className={cn("px-2 text-[11px] font-semibold text-foreground truncate", compact ? "flex-1" : "mt-0.5")}>{item.refNumber}</div>
      <div className={cn("mx-2 mt-1 rounded border border-line/40 bg-surface-2/60", compact && "hidden")}
        style={{ aspectRatio: "16/10" }}>
        <div className="flex h-full w-full items-center justify-center text-[10px] uppercase text-slate/70">{item.format} preview</div>
      </div>
      ...
      <div className={cn("space-y-0.5 p-2 text-[10.5px] text-slate", compact && "p-0")}>
        {item.linkedVesselId && <div className="flex items-center gap-1"><Ship className="h-3 w-3" /> {vesselName(item.linkedVesselId)}</div>}
        {item.linkedPortCode && <div className="flex items-center gap-1"><Anchor className="h-3 w-3" /> {portName(item.linkedPortCode)}</div>}
        {item.linkedInvestigation && <div className="flex items-center gap-1"><FolderOpen className="h-3 w-3" /> {item.linkedInvestigation}</div>}
        <div className="flex items-center gap-1"><span className="h-1 w-1 rounded-full bg-slate" /> {new Date(item.uploadedAt).toLocaleString(undefined, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
        <div className="flex items-center gap-1">{item.uploadedBy}</div>
      </div>
      <div className={cn("mx-2 mb-2 rounded-md py-1 text-center text-[10.5px] font-semibold", tierBg(item.confidence), compact && "hidden")}>
...
function tierBg(tier: EvidenceItem["confidence"]) {
  return tier === "verified" ? "bg-emerald-500/15 text-emerald-300"
    : tier === "observed" ? "bg-amber-500/15 text-amber-300"
    : tier === "inferred" ? "bg-sky-500/15 text-sky-300"
    : "bg-slate-500/15 text-slate";
}

function ConfidenceBadge({ tier, className }: { tier: EvidenceItem["confidence"]; className?: string }) {
  const label = tier.toUpperCase();
  return (
    <span className={cn(
      "inline-flex w-fit items-center gap-1 rounded px-1.5 py-[1px] text-[9.5px] font-semibold uppercase tracking-[0.06em]",
      tier === "verified" ? "bg-emerald-500/15 text-emerald-300"
        : tier === "observed" ? "bg-amber-500/15 text-amber-300"
        : tier === "inferred" ? "bg-sky-500/15 text-sky-300"
        : "bg-slate-500/15 text-slate",
      className,
...
function EvidencePreview({ item }: { item: EvidenceItem }) {
  ...
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">Evidence Preview</span>
      ...
      <div className="grid grid-cols-12 gap-3 p-3">
        ...
        <div className="col-span-12 md:col-span-4">
          <div className="rounded-md border border-line/60 bg-white/95 p-3 text-slate-900" style={{ aspectRatio: "3/4" }}>
            ...
            <div className="space-y-1">
              {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} className="h-1 w-full rounded bg-slate-200" style={{ width: `${80 + ((i * 7) % 20)}%` }} />
              ))}
            </div>
          </div>
          <div className="mt-2 flex items-center justify-between text-[10.5px] text-slate">
            <div className="flex items-center gap-1">
              <button className="rounded p-1 hover:bg-surface-2/60"><ZoomOut className="h-3 w-3" /></button>
              <button className="rounded border border-line/60 px-1.5 py-0.5">Fit Width <ChevronDown className="inline h-3 w-3" /></button>
              <button className="rounded p-1 hover:bg-surface-2/60"><ZoomIn className="h-3 w-3" /></button>
            </div>
            <span>1 / 3</span>
            <div className="flex items-center gap-1">
              <button className="rounded p-1 hover:bg-surface-2/60"><ChevronLeft className="h-3 w-3" /></button>
              <button className="rounded p-1 hover:bg-surface-2/60"><ChevronRight className="h-3 w-3" /></button>
              <button className="rounded p-1 hover:bg-surface-2/60"><Download className="h-3 w-3" /></button>
              <button className="rounded p-1 hover:bg-surface-2/60"><Maximize2 className="h-3 w-3" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <dl className="space-y-1.5 text-[11.5px]">
            ...
            <RowKV k="Confidence Score" v={<span className="text-emerald-300 font-semibold">{item.confidenceScore}%</span>} />
            ...
            <div className="flex items-start justify-between gap-2 pt-1">
              ...
              <div className="flex flex-wrap justify-end gap-1">
                {item.tags.map((t) => (
                  <span key={t} className="rounded-full bg-[color:var(--color-blue)]/15 px-2 py-[1px] text-[10px] text-[color:var(--color-blue)]">{t}</span>
                ))}
              </div>
            </div>
          </dl>
          ...
          <div>
            <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.06em] text-slate">Linked Entities</div>
            <ul className="space-y-1.5 text-[11.5px]">
              {item.linkedVesselId && <LinkedRow icon={<Ship className="h-3 w-3" />} label="Vessel" name={vesselName(item.linkedVesselId)!} meta="IMO 9837456" />}
              {item.linkedCompany && <LinkedRow icon={<Building2 className="h-3 w-3" />} label="Company" name={item.linkedCompany} meta="RC 556677" />}
              {item.linkedPortCode && <LinkedRow icon={<Anchor className="h-3 w-3" />} label="Port" name={portName(item.linkedPortCode)!} meta="Lagos, Nigeria" />}
              {item.linkedVoyage && <LinkedRow icon={<ArrowRight className="h-3 w-3" />} label="Voyage" name={item.linkedVoyage} />}
            </ul>
            <button className="mt-2 text-[11px] text-[color:var(--color-blue)] hover:underline">View full relationships →</button>

            <div className="mt-3">
              <div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.06em] text-slate">Description</div>
...
function LinkedRow({ icon, label, name, meta }: { icon: React.ReactNode; label: string; name: string; meta?: string }) {
  return (
    <li className="flex items-center justify-between">
      <span className="flex items-center gap-1.5 text-slate">
        <span className="text-[color:var(--color-blue)]">{icon}</span> {label}
      </span>
      <span className="text-right"><span className="text-foreground">{name}</span>{meta && <span className="ml-1 text-[10.5px] text-slate">{meta}</span>}</span>
    </li>
  );
}
...
function EvidenceCopilot({ item }: { item: EvidenceItem }) {
  ...
  const insights = [
    { label: `This Bill of Lading matches 3 previous shipments`, tag: "98%", tone: "text-emerald-300" },
    { label: `Container number appears in 2 other manifests`, tag: "95%", tone: "text-emerald-300" },
    { label: `Consignee company linked to high-risk entity`, tag: "HIGH", tone: "text-rose-300" },
    { label: `Similar documents used in ${item.linkedInvestigation ?? "prior cases"}`, tag: "MEDIUM", tone: "text-amber-300" },
    ...
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">
          ...
          <span className="rounded-full bg-[color:var(--color-blue)]/20 px-1.5 py-[1px] text-[9px] font-bold text-[color:var(--color-blue)]">BETA</span>
        </span>
      </header>
      <div className="space-y-3 p-3">
        <div className="relative">
        ...
        <div>
          <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-slate">Evidence Insights</div>
          <ul className="space-y-1.5">
            {insights.map((i, idx) => (
              <li key={idx} className="flex items-start justify-between gap-2 rounded-md border border-line/40 bg-surface-2/30 p-2 text-[11px]">
                <span className="flex items-start gap-1.5 text-foreground/90"><CheckCircle2 className="mt-[1px] h-3 w-3 text-emerald-400" /> {i.label}</span>
                <span className={cn("shrink-0 font-semibold", i.tone)}>{i.tag}</span>
              </li>
            ))}
          </ul>
          <button className="mt-2 text-[11px] text-[color:var(--color-blue)] hover:underline">View full insights →</button>
        </div>
        <p className="text-[10px] italic text-slate">
          Copilot surfaces observations. Officer decides. All recommendations carry OC-001 confidence.
        </p>
      </div>
    </section>
  );
...
function SimilarEvidence({ current, all, onSelect }: { current: EvidenceItem; all: EvidenceItem[]; onSelect: (id: string) => void }) {
  const items = all.filter((e) => e.id !== current.id && e.kind === current.kind).slice(0, 3);
  const scores = [98, 96, 95];
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">Similar Evidence</span>
        <button className="text-[10.5px] text-[color:var(--color-blue)] hover:underline">View all</button>
      </header>
      <ul className="divide-y divide-line/40">
        {items.length === 0 ? (
          ...
          <li key={e.id}>
            <button onClick={() => onSelect(e.id)} className="flex w-full items-center justify-between gap-2 px-3 py-2 text-left hover:bg-surface-2/50">
              <span className="flex items-center gap-2 text-[11px]">
                <FileText className="h-3 w-3 text-[color:var(--color-blue)]" />
                <span className="font-mono text-foreground">{e.refNumber}</span>
              </span>
              <span className="text-[10.5px] text-slate">{new Date(e.uploadedAt).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</span>
...
function InvestigationUsage() {
  const items = [
    ...
    { id: "INV-2026-00521", title: "Sanctions Evasion", status: "ACTIVE", tone: "text-emerald-300" },
  ];
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">Investigation Usage</span>
        <button className="text-[10.5px] text-[color:var(--color-blue)] hover:underline">View all</button>
      </header>
      <ul className="divide-y divide-line/40">
        {items.map((i) => (
...
function ChainOfCustody({ item }: { item: EvidenceItem }) {
  const steps: EvidenceItem["custody"][number]["step"][] = [
    "Uploaded", "Verified", "Reviewed", "Referenced by AI", "Used in Investigation", "Officer Decision", "Shared", "Archived",
  ];
          ...
          return (
            <div key={s} className="flex flex-col items-center text-center">
              <div className={cn(
                ...
                complete ? "border-emerald-400 bg-emerald-500/15 text-emerald-300" : "border-line/60 bg-surface-2/40 text-slate",
              )}>
                {complete ? <CheckCircle2 className="h-4 w-4" /> : <span className="text-[10px]">—</span>}
              </div>
              <div className="mt-1 text-[10px] font-semibold uppercase tracking-[0.05em] text-foreground/90">{s}</div>
              <div className="mt-0.5 text-[9.5px] text-slate">{rec ? new Date(rec.at).toLocaleString(undefined, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"}</div>
...
function AuditTrailPanel() {
  return (
    <section className="h-full rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">Audit Trail</span>
        <button className="text-[10.5px] text-[color:var(--color-blue)] hover:underline">View full audit trail →</button>
      </header>
      <ul className="divide-y divide-line/40">
        {AUDIT_ENTRIES.map((a, i) => (
          <li key={i} className="grid grid-cols-[110px_1fr_auto] items-center gap-2 px-3 py-1.5 text-[11px]">
            <span className="text-slate">{new Date(a.at).toLocaleString(undefined, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
    ...
    <section className="h-full rounded-lg border border-line/60 bg-surface-1/70">
          ...
          <ResponsiveContainer>
            <PieChart>
              <Pie data={counts} innerRadius={40} outerRadius={58} dataKey="value" stroke="none">
                {counts.map((c) => <Cell key={c.name} fill={c.color} />)}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
        ...
        <ul className="space-y-1 text-[11px]">
          {counts.map((c) => (
            <li key={c.name} className="flex items-center justify-between">
              ...
              <span className="text-slate">{fmt(c.value)} ({Math.round((c.value / total) * 100)}%)</span>
            </li>
          ))}
        </ul>
      </div>
      <div className="border-t border-line/60 px-3 py-2 text-[10.5px] text-slate">
        ...
        <div className="mt-1 h-1.5 w-full overflow-hidden rounded bg-surface-2/50">
          <div className="h-full rounded bg-[color:var(--color-blue)]/80" style={{ width: "24.8%" }} />
        </div>
      </div>
    </section>
  );
...
function RelationshipsPanel({ evidence }: { evidence: EvidenceItem[] }) {
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70 p-4">
      <div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">Evidence Relationships</div>
      <p className="mt-2 text-[11.5px] text-foreground/80">
        {evidence.length} evidence items linked across vessels, ports, companies and investigations. Full relationship graph is available via the Knowledge Graph in Institutional Memory.
      </p>
      <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3">
        {evidence.slice(0, 6).map((e) => (
          <div key={e.id} className="rounded-md border border-line/40 bg-surface-2/40 p-2 text-[11px]">
      ...
      <ul className="divide-y divide-line/40">
        {[...byInv.entries()].map(([inv, items]) => (
          <li key={inv} className="flex items-center justify-between px-3 py-2 text-[11.5px]">
            ...
            <span className="text-slate">{items.map((i) => i.kind).slice(0, 3).join(", ")}</span>
          </li>
        ))}
      </ul>
    </section>
  ...
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      ...
      <ol className="space-y-2 p-3">
        {sorted.map((e) => (
          <li key={e.id} className="flex items-start gap-3 rounded-md border border-line/40 bg-surface-2/30 p-2">
            ...
            <div className="flex-1">
              ...
              <div className="text-[10.5px] text-slate">{new Date(e.uploadedAt).toLocaleString()} · {e.uploadedBy} · {e.kind}</div>
            </div>
          </li>
        ))}
      </ol>
...
function AuditPanel() {
  return (
    <section className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">Full Audit Trail (HR-9)</span>
...
function UploadEvidenceModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-lg border border-line/60 bg-surface-1 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <header className="flex items-center justify-between border-b border-line/60 px-4 py-3">
          <div>
            ...
            <div className="text-[11px] text-slate">Versioned. Classified. Validated. Immutable audit history.</div>
          </div>
        ...
        <div className="flex items-center gap-2 border-b border-line/60 px-4 py-2 text-[10.5px] uppercase tracking-[0.06em] text-slate">
          {["Select File", "Classify", "Validate", "Submit"].map((s, i) => (
            <span key={s} className={cn("flex items-center gap-1", i === step && "text-[color:var(--color-blue)]", i < step && "text-emerald-400")}>
              <span className={cn("flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-bold",
                i === step ? "bg-[color:var(--color-blue)]/20 text-[color:var(--color-blue)]" :
                i < step ? "bg-emerald-500/20 text-emerald-300" : "bg-surface-2/60 text-slate",
              )}>{i + 1}</span>
              {s}
        ...
        <div className="space-y-3 p-4">
          {step === 0 && (
            <label className="flex h-40 cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-line/60 bg-surface-2/40 hover:bg-surface-2/60">
              ...
              <div className="mt-2 text-[12px] font-semibold text-foreground">{file ? file.name : "Drop file or click to browse"}</div>
              <div className="text-[10.5px] text-slate">PDF, JPG, PNG, CSV, XML, JSON · up to 25 MB</div>
          ...
          {step === 1 && (
            <div className="grid grid-cols-2 gap-3 text-[11.5px]">
              <FieldSelect label="Evidence Type" value={kind} onChange={setKind} options={TYPE_OPTIONS} />
              <FieldSelect label="Classification" value={classification} onChange={setClassification} options={CLASSIFICATION_OPTIONS} />
              <FieldInput label="Linked Investigation" value={investigation} onChange={setInvestigation} placeholder="INV-2026-00431" />
              <FieldInput label="Tags (comma separated)" value={tags} onChange={setTags} placeholder="revenue, import" />
            </div>
          )}
          ...
          {step === 2 && (
            <ul className="space-y-2 text-[11.5px]">
              {[
                ...
                <li key={i} className="flex items-center gap-2 rounded border border-line/40 bg-surface-2/30 px-3 py-2">
                  <CheckCircle2 className={cn("h-4 w-4", v.ok ? "text-emerald-400" : "text-slate")} />
          ...
          {step === 3 && (
            <div className="rounded-md border border-line/40 bg-surface-2/30 p-3 text-[11.5px]">
              ...
              <p className="mt-3 text-[10.5px] italic text-slate">
                Submitting will create an immutable audit entry (HR-9) and version 1.0 of this evidence.
              </p>
            </div>
          )}
        </div>
...
function FieldInput({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <label className="block">
      <span className="mb-1 block text-[10.5px] uppercase tracking-[0.06em] text-slate">{label}</span>
      <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-md border border-line/50 bg-surface-2/40 px-2 py-1.5 text-[11.5px] text-foreground placeholder:text-slate focus:outline-none focus:ring-1 focus:ring-[color:var(--color-blue)]" />
    </label>
  );
}
function FieldSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: string[] }) {
  return (
    <label className="block">
      <span className="mb-1 block text-[10.5px] uppercase tracking-[0.06em] text-slate">{label}</span>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-md border border-line/50 bg-surface-2/40 px-2 py-1.5 text-[11.5px] text-foreground focus:outline-none focus:ring-1 focus:ring-[color:var(--color-blue)]">
