...
function facet(
  items: EvidenceItem[],
  pick: (i: EvidenceItem) => string | string[] | undefined | null,
): FacetOption[] {
...
export function deriveFacets(items: EvidenceItem[]): FacetSet {
  return {
    ...
    entities: (() => {
      ...
      for (const it of items) {
        if (it.linkedVesselId)
          push(
            `v:${it.linkedVesselId}`,
            `Vessel · ${vesselName(it.linkedVesselId) ?? it.linkedVesselId}`,
          );
...
export function applyEvidenceFilters(
  ...
  return items.filter((e) => {
    if (q) {
      const hay =
        `${e.refNumber} ${e.kind} ${e.description} ${e.tags.join(" ")} ${e.uploadedBy} ${e.source} ${e.linkedInvestigation ?? ""}`.toLowerCase();
    ...
    if (
      f.investigations.length &&
      (!e.linkedInvestigation || !f.investigations.includes(e.linkedInvestigation))
    )
      return false;
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            <ChevronDown
              className={cn(
                "h-3 w-3 shrink-0 text-slate transition-transform",
                open && "rotate-180",
              )}
            />
          </button>
        ...
        <PopoverContent
          align="start"
          sideOffset={4}
          className="w-[260px] border-line/70 bg-surface-1 p-0 text-foreground"
        >
    ...
    <div>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          align="start"
          sideOffset={4}
          className="w-auto max-w-[92vw] border-line/70 bg-surface-1 p-0 text-foreground"
        >
...
export function EvidenceFilterSidebar({
  items,
  filters,
  setFilters,
  loading,
  ...
  const CLASSIFICATIONS: FacetOption[] = [
    "Official Document",
    "Field Capture",
    "System Ingest",
    "Third-Party Feed",
  ...
  const rows: { key: string; label: string; node: React.ReactNode }[] = [
    ...
    },
    {
      ...
      node: (
        <MultiSelectFilter
          label="Investigation"
          hint="All Investigations"
          options={facets.investigations}
          selected={filters.investigations}
    ...
    },
    {
      ...
      node: (
        <DateRangeFilter
          ...
          to={filters.dateTo}
          onChange={(f, t) => patch({ dateFrom: f, dateTo: t })}
        />
      ),
    },
    {
      ...
      node: (
        <MultiSelectFilter
          label="Uploaded By"
          hint="All Officers"
          options={facets.officers}
          selected={filters.officers}
          onChange={(v) => patch({ officers: v })}
          loading={loading}
          error={error}
        />
      ),
    },
    {
      ...
      node: (
        <MultiSelectFilter
          ...
          onChange={(v) => patch({ classifications: v })}
        />
      ),
    },
    {
    ...
    <aside className="rounded-lg border border-line/60 bg-surface-1/70">
      <header className="flex items-center justify-between border-b border-line/60 px-3 py-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate">
          Filter Evidence
          {active > 0 && (
            <span className="ml-1.5 rounded bg-[color:var(--color-blue)]/20 px-1.5 py-0.5 text-[10px] text-[color:var(--color-blue)]">
              {active}
            </span>
          )}
        </span>
        <button
          onClick={clear}
          className={cn(
            "text-[10.5px] hover:underline",
            active > 0 ? "text-[color:var(--color-blue)]" : "text-slate/60 cursor-default",
          )}
          disabled={active === 0}
        >
          Clear All
      ...
      <div className="space-y-3 p-3">
        ...
        <button
          onClick={() => {
            /* filters apply live — this is a visual confirmation */
          }}
