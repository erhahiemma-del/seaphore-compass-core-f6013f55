...
const KPIS: KpiSpec[] = [
  ...
  },
  {
    ...
    trend: "up",
    confidence: "inferred",
    series: sparkSeries(47),
    emphasis: "risk",
  },
  {
    label: "Misclassified Cargo",
    value: "9",
    delta: "+2",
    trend: "up",
    confidence: "observed",
    series: sparkSeries(53),
  },
  {
    ...
    <IntelCentreShell
      ...
      tabs={[
        ...
        { key: "hs", label: "HS Codes" },
        {
          key: "dg",
          label: "Dangerous Goods",
          count: CARGO.filter((c) => c.dangerousGoods).length,
        },
      ...
      filters={
        <>
          ...
          <FilterBlock label="Origin country">
            <CheckList
              options={["China", "Netherlands", "UAE", "Russia", "India"]}
              defaultChecked={["China", "UAE"]}
            />
          </FilterBlock>
          <FilterBlock label="Destination port">
            <CheckList options={PORTS.map((p) => p.name)} defaultChecked={["Apapa Port"]} />
          </FilterBlock>
          <FilterBlock label="Cargo type">
            <CheckList options={CARGO_TYPE_MIX.map((t) => t.type)} />
          </FilterBlock>
      ...
      copilot={
        <CentreCopilot
          ...
          observed={[
            {
              title: "HS-code drift on mobile phones",
              detail: "Observed 8517.12 vs 8471.30 across 4 containers ex-China.",
              confidence: "observed",
            },
            {
              title: "Weight-value ratio anomaly",
              detail: "Container MSCU7811203 declared 40% below HS-code median.",
              confidence: "inferred",
            },
            {
              title: "Recurring consignee address",
              detail: "3 shipments to same address under different importers.",
              confidence: "observed",
            },
          ]}
          recommendations={[
            {
              title: "Hold MSCU7811203 for physical inspection",
              detail: "Confidence 82 · duty exposure ₦88M.",
              confidence: "inferred",
            },
            {
              title: "Re-verify HS 8517.12 declarations",
              detail: "Cross-check with WCO reference values.",
              confidence: "observed",
            },
            {
              title: "Flag HLXU3320041 for DG audit",
              detail: "IMDG Class 3 flammable observed on general-cargo manifest.",
              confidence: "observed",
            },
          ]}
          historical={[
            {
              title: "Container CMAU9982017 · Q3 2025",
              detail: "Similar mobile-phone under-declaration → ₦210M recovery.",
              similarity: 79,
            },
            {
              title: "HLXU3320041 · Feb 2026",
              detail: "Chemical mis-classification pattern matched HS-2933.99.",
              similarity: 64,
            },
          ]}
          related={[
  ...
  return (
    <div className="relative overflow-hidden rounded border border-line/60 bg-[#0A1524]">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-[320px] w-full">
        ...
        <text x={dest.x + 10} y={dest.y + 4} fill="#8DA5C7" fontSize="11" fontWeight={600}>
          Nigeria
        </text>
        ...
        {origins.map((o, i) => {
          ...
          const y = 40 + (i % 3) * 80;
          const colour =
            o.risk === "high" ? "#C0392B" : o.risk === "medium" ? "#B06A00" : "#1E6B3A";
          return (
            <g key={o.country}>
              <path
              ...
              <text x={x + 7} y={y + 4} fill="#8DA5C7" fontSize="10">
                {o.country}
              </text>
            </g>
          );
...
function CargoSummary() {
  ...
  const exposure = CARGO.filter((c) => c.riskLevel !== "low").reduce(
    (a, b) => a + b.declaredValueNGN * 0.06,
    0,
  );
  const rows = [
  ...
  return (
    <div className="flex items-center gap-4">
      <svg viewBox="0 0 180 180" className="h-[180px] w-[180px]">
        ...
        <text x="90" y="88" textAnchor="middle" fill="#E4E8EC" fontSize="18" fontWeight={700}>
          {CARGO.length}
        </text>
        <text x="90" y="104" textAnchor="middle" fill="#5A6B7B" fontSize="10">
          items
        </text>
      </svg>
  ...
  return [
    {
      ...
      render: (r) => (
        <span
          className={`font-mono text-[11.5px] ${r.containerNo === selected ? "text-[color:var(--color-blue)]" : ""}`}
        >
          {r.containerNo}
        </span>
      ),
    ...
    { key: "vs", label: "Vessel", render: (r) => vesselById(r.vesselId)?.name ?? "—" },
    {
      key: "hs",
      label: "HS",
      render: (r) => <span className="font-mono text-[11.5px]">{r.hsCode}</span>,
    },
    {
      key: "de",
      label: "Description",
    ...
    { key: "or", label: "Origin", render: (r) => r.origin },
    {
      key: "dt",
      label: "Dest.",
      render: (r) => portByCode(r.destination)?.name.replace(" Port", ""),
    },
    ...
    { key: "wt", label: "MT", align: "right", render: (r) => r.weightMT.toLocaleString() },
    {
      ...
      render: (r) => (
        <StatusBadge
          ...
          tone={r.riskLevel === "high" ? "risk" : r.riskLevel === "medium" ? "warn" : "ok"}
        />
      ),
    },
    {
      ...
      label: "Score",
      align: "right",
      render: (r) => <span className="font-semibold">{r.riskScore}</span>,
    },
    {
      key: "st",
      label: "Status",
      render: (r) => (
        <StatusBadge
          label={r.status}
          tone={
            r.status === "Held" || r.status === "Discrepancy"
              ? "risk"
              : r.status === "Inspection"
                ? "warn"
                : "ok"
