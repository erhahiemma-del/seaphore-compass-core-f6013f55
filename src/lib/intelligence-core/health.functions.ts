...
export const getMicHealthFn = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    const status =
      ...
      : latest === null ? "idle"
      : latest.outcome === "failed" ? "degraded"
      : latest.outcome === "degraded" ? "warning"
      : "healthy";
...
export const getMicMetricsFn = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    const latest = mioCaptureSink.latest;
    const heapMb = latest?.heapUsedBytes != null
      ? +(latest.heapUsedBytes / 1_048_576).toFixed(1)
      : null;
    ...
    const p50 = durations.length > 0
      ? durations[Math.floor(durations.length * 0.50)]
      : null;
