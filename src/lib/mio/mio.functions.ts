  ...
  .handler(async () => {
    ...
    return {
      executions: executions.slice(0, 100).map((e) => ({
        ...
        heapUsedMb:              e.heapUsedBytes != null
          ? +(e.heapUsedBytes / 1_048_576).toFixed(1) : null,
        warnings: e.warnings,
        errors:   e.errors,
      })),
...
export const getMioPipelineStatusFn = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    return {
      pipeline: [
        { stage: "Evidence Providers", status: "healthy",                      latencyMs: null, note: "Managed by Provider Health" },
        ...
        { stage: "MIC",                status: latest?.outcome ?? "not-run",   latencyMs: latest?.totalDurationMs ?? null, note: latest ? `Last run: ${latest.timestamp}` : "No executions yet" },
        ...
        { stage: "Officer Dashboard",  status: "healthy",                      latencyMs: null, note: "Canonical UIP projections" },
        { stage: "MIBC",               status: "healthy",                      latencyMs: null, note: "PDF/DOCX/XLSX/PPTX" },
      ],
      micStages: (latest?.stageTimings ?? []).map((t) => ({
        stage:      t.stage,
        durationMs: t.durationMs,
        status:     latest?.outcome === "failed" ? "critical"
                  : latest?.outcome === "degraded" ? "warning" : "healthy",
      })),
      summary,
      timestamp: new Date().toISOString(),
    };
...
export const getMioRiskDistributionFn = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    return {
      ...
      topRisk: allRisk.sort((a, b) => b.score - a.score).slice(0, 10).map((r) => ({
        entityId:    r.entityId,
