 ...
 *
 * Produces a single-shot PDF summarising the current screening queue:
 ...
 *
 * No server round-trip. Mirrors the on-screen tracker so the report is
...
function drawFooter(pdf: jsPDF, page: number) {
  ...
  pdf.line(MARGIN_X, h - 42, w - MARGIN_X, h - 42);
  setColor(pdf, "text", COLORS.muted);
  pdf.setFont("helvetica", "italic");
  pdf.setFontSize(8);
  pdf.text("Evidence first. Explainable always. Officer decides.", MARGIN_X, h - 28);
  pdf.setFont("helvetica", "normal");
  pdf.text(`Page ${page}`, w - MARGIN_X, h - 28, { align: "right" });
}

function ensureSpace(pdf: jsPDF, cursor: Cursor, needed: number) {
  const h = pdf.internal.pageSize.getHeight();
  if (cursor.y + needed > h - MARGIN_BOTTOM) {
...
export function buildComplianceReportPdf(input: ComplianceReportInput): { pdf: jsPDF; filename: string } {
  ...
  writeSectionTitle(cursor.page === 1 ? pdf : pdf, cursor, "Executive Assessment");
  ...
  if (banner) {
    ...
    pdf.setFillColor(...(STATUS_COLOR[posture.worst] as unknown as [number, number, number]));
  ...
  writeParagraph(pdf, cursor, action);
  cursor.y += 6;

  // Evidence summary — per-entity
  writeSectionTitle(pdf, cursor, "Evidence Summary");
  if (rows.length === 0) {
    writeParagraph(pdf, cursor, "No entities were queued for screening at report time.");
  } else {
    rows.forEach((r, i) => {
      ...
      if (r.summary) {
        setColor(pdf, "text", COLORS.ink);
        pdf.setFontSize(10);
        writeParagraph(pdf, cursor, r.summary, 10);
      }
      if (r.error) {
        setColor(pdf, "text", COLORS.danger);
        writeParagraph(pdf, cursor, `Error: ${r.error}`, 9.5);
      }

      // Divider
      setColor(pdf, "draw", COLORS.rule);
      pdf.setLineWidth(0.3);
      pdf.line(MARGIN_X, cursor.y + 2, pdf.internal.pageSize.getWidth() - MARGIN_X, cursor.y + 2);
      cursor.y += 12;
    });
  }
